...
const contentManager = new ContentManager(dataEntityRepo, collectionRepo, dataBindingRepo);
...
const storeFactory = new StoreFactory(storeRepo, dataEntityRepo, themeRepo, seeder, synchronizer);
const sallaValidator = new SallaValidator();
const hookService = new HookService();
const localizationService = new LocalizationService();
const engine = new CompositionEngine(storeRepo, themeRegistry, contentManager, storeLogic, hookService, localizationService);
...
// 1. Health & Debug
app.get('/api/health', (req, res) => ok(res, { vtdr: 'active', timestamp: new Date().toISOString() }));
app.get('/api/debug/test', (req, res) => ok(res, { message: 'API is reachable' }));

// 2. Salla Assets Proxy
app.use(proxy('https://cdn.salla.network', {
...
app.use((err: any, req: any, res: any, next: any) => {
    ...
    res.status(err.status || 500).send(`<h1>System Error</h1><p>${err.message}</p>`);
});
