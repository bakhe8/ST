...
function normalizeDeepLinkCandidate(rawPath: unknown): string {
  ...
  if (pathOnly.startsWith("/preview") || pathOnly.startsWith("/themes"))
    return "";
...
describe("Theme Runtime Contract v1 compatibility", () => {
  ...
  beforeAll(async () => {
    ...
    dbFile = path.join(
      os.tmpdir(),
      `vtdr-contract-${Date.now()}-${Math.random().toString(36).slice(2)}.db`,
    );
  ...
  it("validates canonical preview/runtime contract for every discovered theme", async () => {
    ...
    for (const theme of themes) {
      ...
      if (createStoreRes.status !== 200) {
        failures.push(
          `[${themeId}] Failed to create store (status=${createStoreRes.status})`,
        );
        continue;
      }
      ...
      if (updateStoreRes.status !== 200) {
        failures.push(
          `[${themeId}] Failed to bind store to theme (status=${updateStoreRes.status})`,
        );
        continue;
      }
      ...
      const fetchEntityCollections = async () => {
        const [productsRes, categoriesRes, brandsRes, blogArticlesRes] =
          await Promise.all([
            ...
            fetch(`${baseUrl}/api/v1/blog/articles`, {
        ...
        if (productsRes.status !== 200) {
          failures.push(
            `[${themeId}] GET /api/v1/products failed (status=${productsRes.status})`,
          );
        } else {
        ...
        if (categoriesRes.status !== 200) {
          failures.push(
            `[${themeId}] GET /api/v1/categories failed (status=${categoriesRes.status})`,
          );
        } else {
          const categoriesJson: any = await categoriesRes.json();
          collections.categories = extractArrayData(
            categoriesJson,
          ) as JsonMap[];
        }
        ...
        if (brandsRes.status !== 200) {
          failures.push(
            `[${themeId}] GET /api/v1/brands failed (status=${brandsRes.status})`,
          );
        } else {
        ...
        if (blogArticlesRes.status !== 200) {
          failures.push(
            `[${themeId}] GET /api/v1/blog/articles failed (status=${blogArticlesRes.status})`,
          );
        } else {
          const blogArticlesJson: any = await blogArticlesRes.json();
          collections.blogArticles = extractArrayData(
            blogArticlesJson,
          ) as JsonMap[];
        }
      ...
      let { products, categories, brands, blogArticles } =
      ...
      if (
        ...
        if (seedRes.status !== 200) {
          failures.push(
            `[${themeId}] POST /api/stores/${storeId}/seed failed (status=${seedRes.status})`,
          );
        } else {
      ...
      const menusRes = await fetch(`${baseUrl}/api/v1/menus/header`, {
        headers: contextHeaders,
      });
      if (menusRes.status !== 200) {
        failures.push(
          `[${themeId}] GET /api/v1/menus/header failed (status=${menusRes.status})`,
        );
      } else {
      ...
      if (componentsRes.status !== 200) {
        failures.push(
          `[${themeId}] GET /api/v1/theme/components failed (status=${componentsRes.status})`,
        );
      } else {
        const componentsJson: any = await componentsRes.json();
        if (!Array.isArray(componentsJson?.data?.components)) {
          failures.push(
            `[${themeId}] Theme components payload is not normalized`,
          );
        }
      }

      const settingsRes = await fetch(`${baseUrl}/api/v1/theme/settings`, {
        headers: contextHeaders,
      });
      if (settingsRes.status !== 200) {
        failures.push(
          `[${themeId}] GET /api/v1/theme/settings failed (status=${settingsRes.status})`,
        );
      }
      ...
      if (homePreviewRes.status !== 200) {
        failures.push(
          `[${themeId}] Preview index failed (status=${homePreviewRes.status})`,
        );
      } else {
        ...
        if (
          ...
          failures.push(
            `[${themeId}] Preview index returned render error marker`,
          );
        }
        if (!homeHtml.includes("/sdk-bridge.js")) {
          failures.push(
            `[${themeId}] sdk-bridge injection missing from preview HTML`,
          );
        }
        if (!homeHtml.includes("window.__VTDR_NAV_SHIM__")) {
          failures.push(
            `[${themeId}] navigation shim marker missing from preview HTML`,
          );
        }
        if (!homeHtml.includes("window.vtdr_context")) {
          failures.push(
            `[${themeId}] vtdr runtime context script missing from preview HTML`,
          );
        }
        if (!homeHtml.includes("window.__VTDR_PREVIEW_NAV__")) {
          failures.push(
            `[${themeId}] shared preview navigation API missing from preview HTML`,
          );
        }
        if (!homeHtml.includes("__VTDR_API_CONTEXT_SHIM__")) {
          failures.push(
            `[${themeId}] preview API context shim marker missing from preview HTML`,
          );
        }
        if (!homeHtml.includes('"api":"/api/v1"')) {
          failures.push(
            `[${themeId}] preview store.api is not normalized to /api/v1`,
          );
        }
        if (homeHtml.includes('"api":"http://localhost:3001/api/v1"')) {
          failures.push(
            `[${themeId}] preview store.api still hardcoded to http://localhost:3001/api/v1`,
          );
        }
        const previewBasePath = `/preview/${storeId}/${themeId}/${themeVersion}`;
        if (
          !homeHtml.includes(`href="${previewBasePath}`) &&
          !homeHtml.includes(`href='${previewBasePath}`)
        ) {
          failures.push(
            `[${themeId}] home HTML missing preview-bound anchor href`,
          );
        }
        if (
          homeHtml.includes('href="http://localhost:3001/"') ||
          homeHtml.includes("href='http://localhost:3001/'")
        ) {
          failures.push(
            `[${themeId}] home HTML still contains hardcoded localhost root href`,
          );
        }
      }
      ...
      if (queryScopedProductsRes.status !== 200) {
        failures.push(
          `[${themeId}] GET /api/v1/products via store_id query context failed (status=${queryScopedProductsRes.status})`,
        );
      } else {
        const queryScopedProductsJson: any =
          await queryScopedProductsRes.json();
        const queryScopedProducts = extractArrayData(queryScopedProductsJson);
        if (
          !Array.isArray(queryScopedProducts) ||
          queryScopedProducts.length === 0
        ) {
          failures.push(
            `[${themeId}] GET /api/v1/products via store_id query context returned empty data`,
          );
        }
      }
      ...
      if (!crossThemePreviewValidated && themes.length > 1) {
        const alternateTheme = themes.find(
          (entry: any) => String(entry?.id || "") !== themeId,
        );
        const alternateVersion = Array.isArray(alternateTheme?.versions)
          ? alternateTheme.versions[0]
          : null;
        const alternateThemeId = String(alternateTheme?.id || "").trim();
        const alternateThemeVersion = String(
          alternateVersion?.version || "",
        ).trim();
      ...
      for (const routePath of PREVIEW_NAVIGATION_PATHS.filter(
        (item) => item !== "/",
      )) {
        ...
        if (previewRes.status !== 200) {
          pushRouteFailure(
            routePath,
            `Preview route failed (status=${previewRes.status})`,
          );
          continue;
        }
        ...
        if (html.includes("Renderer Error") || html.includes("Render Error")) {
          pushRouteFailure(
            routePath,
            "Preview route contains render error marker",
          );
        }
        if (!html.includes("__VTDR_API_CONTEXT_SHIM__")) {
          pushRouteFailure(
            routePath,
            "Preview route missing API context shim marker",
          );
        }
        if (routePath === "/products" && !html.includes('"api":"/api/v1"')) {
          pushRouteFailure(
            routePath,
            "Preview route missing normalized store.api (/api/v1)",
          );
        }
        if (
          routePath === "/products" &&
          html.includes('"api":"http://localhost:3001/api/v1"')
        ) {
          pushRouteFailure(
            routePath,
            "Preview route still hardcoded to localhost API origin",
          );
        }
        ...
        if (
          routePath !== "/" &&
          html.includes('item-id="=desktop&refresh=0"')
        ) {
          pushRouteFailure(
            routePath,
        ...
        if (
          expectedTemplateId &&
          !hasTemplatePageMarker(html, expectedTemplateId)
        ) {
          pushRouteFailure(
            routePath,
        ...
        if (
          ...
          !routeMarkers.some((marker) => html.includes(marker))
        ) {
          pushRouteFailure(
            routePath,
        ...
        if (routePath === "/products") {
          if (products.length === 0) {
            pushRouteFailure(
              routePath,
              "Products API returned no items for products page parity",
            );
          }
          if (!html.includes("<salla-products-list")) {
            pushRouteFailure(
              routePath,
              "Products page missing <salla-products-list> runtime component",
            );
          }
          if (!html.includes('id="page-main-title"')) {
            pushRouteFailure(
              routePath,
              "Products page missing main title container (#page-main-title)",
            );
          }
          if (html.includes("pages.categories.sorting")) {
            pushRouteFailure(
              routePath,
              "Products page leaked raw translation key pages.categories.sorting",
            );
          }
        }
        ...
        if (routePath === "/categories") {
          if (categories.length === 0) {
            pushRouteFailure(
              routePath,
              "Categories API returned no items for categories page parity",
            );
          }
          if (!html.includes("<salla-products-list")) {
            pushRouteFailure(
              routePath,
              "Categories page missing <salla-products-list> runtime component",
            );
          }
          if (!html.includes('id="page-main-title"')) {
            pushRouteFailure(
              routePath,
              "Categories page missing main title container (#page-main-title)",
            );
          }
          if (html.includes("pages.categories.sorting")) {
            pushRouteFailure(
              routePath,
              "Categories page leaked raw translation key pages.categories.sorting",
            );
          }
        }
        ...
        if (routePath === "/brands") {
          if (brands.length === 0) {
            pushRouteFailure(
              routePath,
              "Brands API returned no items for brands page parity",
            );
          }
          if (!html.includes("brands-nav") && !html.includes("brand-item")) {
            pushRouteFailure(
              routePath,
              "Brands page missing brands-nav / brand-item semantic markers",
            );
          }
          ...
          if (brands.length > 0 && html.includes("pages.brands.non_brands")) {
            pushRouteFailure(
              routePath,
              "Brands page leaked empty-state translation key despite existing brands",
            );
          }
        }
        ...
        if (routePath === "/blog") {
          if (blogArticles.length === 0) {
            pushRouteFailure(
              routePath,
              "Blog API returned no articles for blog page parity",
            );
          }
          if (
            !html.includes("post-entry") &&
            !html.includes("blog-slider__slide")
          ) {
            pushRouteFailure(
              routePath,
              "Blog page missing post-entry/blog-slider semantic markers",
            );
          }
          if (html.includes("pages.blog_categories.categories")) {
            pushRouteFailure(
              routePath,
              "Blog page leaked raw translation key pages.blog_categories.categories",
            );
          }
          if (html.includes("pages.blog_categories.no_articles")) {
            pushRouteFailure(
              routePath,
              "Blog page leaked raw translation key pages.blog_categories.no_articles",
            );
          }
        }
      }
      ...
      if (localePreviewRes.status !== 200) {
        failures.push(
          `[${themeId}] Preview localized route /ar/products failed (status=${localePreviewRes.status})`,
        );
      }

      const productEntity = products[0] || {};
      const productToken = pickEntityToken(productEntity, [
        "slug",
        "id",
        "key",
      ]);
      ...
      const categoryToken = pickEntityToken(categoryEntity, [
        "slug",
        "id",
        "key",
      ]);
      ...
      if (deepLinkCandidates.length === 0) {
        failures.push(
          `[${themeId}] no deep-link candidate resolved from simulator entities`,
        );
      }
      ...
      if (bridgeRes.status !== 200) {
        failures.push(
          `[${themeId}] GET /sdk-bridge.js failed (status=${bridgeRes.status})`,
        );
      } else {
        const bridgeText = await bridgeRes.text();
        if (!bridgeText.includes("resolvePreviewBasePath")) {
          failures.push(
            `[${themeId}] sdk-bridge.js missing preview navigation adapter`,
          );
        }
        if (!bridgeText.includes("resolveAnchorFromEvent")) {
          failures.push(
            `[${themeId}] sdk-bridge.js missing composed-path anchor resolver`,
          );
        }
        if (!bridgeText.includes("__VTDR_PREVIEW_NAV__")) {
          failures.push(
            `[${themeId}] sdk-bridge.js missing shared preview navigation API binding`,
          );
        }
      }
    }
    ...
    const parsedFailures = failures
      ...
      .filter(
        (
          entry,
        ): entry is NonNullable<ReturnType<typeof parseThemeFailureEntry>> =>
          Boolean(entry),
      );
    ...
    const themeMetrics = themes.map((theme: any) => {
      ...
      const themeVersion =
        String(theme?.versions?.[0]?.version || "").trim() || "unknown-version";
      const scopedFailures = parsedFailures.filter(
        (entry) => entry.themeId === themeId,
      );
      const themeLevelFailure = scopedFailures.some(
        (entry) => !entry.routePath,
      );

      const coreRoutes = PREVIEW_CORE_PATHS.reduce(
        (acc, routePath) => {
          const hasRouteFailure = scopedFailures.some(
            (entry) => entry.routePath === routePath,
          );
          acc[routePath] =
            themeLevelFailure || hasRouteFailure ? "fail" : "pass";
          return acc;
        },
        {} as Record<string, "pass" | "fail">,
      );

      const routesPassed = Object.values(coreRoutes).filter(
        (status) => status === "pass",
      ).length;
      const routesFailed = Object.values(coreRoutes).filter(
        (status) => status === "fail",
      ).length;
    ...
    const totalRoutesPassed = themeMetrics.reduce(
      (sum: number, item: { routesPassed: number }) =>
        sum + Number(item.routesPassed || 0),
      0,
    );
    const totalRoutesFailed = themeMetrics.reduce(
      (sum: number, item: { routesFailed: number }) =>
        sum + Number(item.routesFailed || 0),
      0,
    );
    const successRatio =
      totalRoutesChecked > 0 ? totalRoutesPassed / totalRoutesChecked : 0;
