...
describe("VTDR API integration (Store-First)", () => {
  ...
  beforeAll(async () => {
    ...
    dbFile = path.join(
      os.tmpdir(),
      `vtdr-int-${Date.now()}-${Math.random().toString(36).slice(2)}.db`,
    );
  ...
  it("covers themes -> stores -> simulator -> preview -> theme settings", async () => {
    const syncRes = await fetch(`${baseUrl}/api/themes/sync`, {
      method: "POST",
    });
    ...
    const selectedProductHref = String(
      productsJson.data[0]?.url || `/products/${selectedProductId}`,
    );
    expect(selectedProductId).toBeTruthy();

    const themeComponentsRes = await fetch(
      `${baseUrl}/api/v1/theme/components`,
      {
        headers: contextHeaders,
      },
    );
    ...
    const homeBrands = themeComponentsJson.data.components.find(
      (c: any) => c.path === "home.brands",
    );
    const homeMainLinks = themeComponentsJson.data.components.find(
      (c: any) => c.path === "home.main-links",
    );
    const homeSquareBanners = themeComponentsJson.data.components.find(
      (c: any) => c.path === "home.enhanced-square-banners",
    );
    const homeSliderProducts = themeComponentsJson.data.components.find(
      (c: any) => c.path === "home.slider-products-with-header",
    );
    const variableListFields = (
      themeComponentsJson.data.components || []
    ...
    const categoriesField = homeMainLinks?.fields?.find(
      (f: any) => f.id === "categories",
    );
    const productsField = homeSliderProducts?.fields?.find(
      (f: any) => f.id === "products",
    );
    ...
    expect(
      Array.isArray(blogArticlesVariableField?.variableOptions?.blog_articles),
    ).toBe(true);
    expect(
      (blogArticlesVariableField?.variableOptions?.blog_articles || []).length,
    ).toBeGreaterThan(0);
    expect(
      Array.isArray(
        blogCategoriesVariableField?.variableOptions?.blog_categories,
      ),
    ).toBe(true);
    expect(
      (blogCategoriesVariableField?.variableOptions?.blog_categories || [])
    ...
    const saveThemeSettingsRes = await fetch(
      `${baseUrl}/api/v1/theme/settings`,
      {
        method: "PUT",
        headers: {
    ...
    const homeComponent = (componentsJson.data.components || []).find(
      (c: any) =>
    ...
    const mainLinksComponentId = String(
      homeMainLinks?.key || homeMainLinks?.path || "",
    );
    const squareBannersComponentId = String(
      homeSquareBanners?.key || homeSquareBanners?.path || "",
    );
    ...
    const createVisualCategoryRes = await fetch(
    ...
    const savePageCompositionRes = await fetch(
      `${baseUrl}/api/v1/theme/settings`,
      {
        method: "PUT",
        headers: {
              ...
              },
              {
                ...
                props: {
                  banners: [
                    {
                      "banners.image":
                        "https://cdn.salla.network/vtdr/banner.jpg",
    ...
    const getThemeSettingsRes = await fetch(
      `${baseUrl}/api/v1/theme/settings`,
      {
        headers: contextHeaders,
      },
    ...
    const savedHomeCompositions =
      getThemeSettingsJson.data.values.page_compositions.home;
    expect(Array.isArray(savedHomeCompositions)).toBe(true);
    expect(
      savedHomeCompositions.some(
        (entry: any) => entry.componentId === String(homeComponent.key),
      ),
    ).toBe(true);
    expect(
      savedHomeCompositions.some(
        (entry: any) => entry.componentId === mainLinksComponentId,
      ),
    ).toBe(true);
  ...
  it("auto-seeds by default and supports canonical seed profiles on create/reseed", async () => {
    const syncThemesRes = await fetch(`${baseUrl}/api/themes/sync`, {
      method: "POST",
    });
    ...
    const categoriesBefore = Array.isArray(categoriesJson.data)
      ? categoriesJson.data
      : [];
    const categoryNamesBefore = categoriesBefore.map((item: any) =>
      String(item?.name || ""),
    );
    ...
    const categoriesAfterRes = await fetch(`${baseUrl}/api/v1/categories`, {
      headers: contextHeaders,
    });
    ...
    const categoriesAfter = Array.isArray(categoriesAfterJson.data)
      ? categoriesAfterJson.data
      : [];
    const categoryNamesAfter = categoriesAfter.map((item: any) =>
      String(item?.name || ""),
    );
  ...
  it("normalizes theme component sources for incomplete theme schemas via API", async () => {
    ...
    try {
      ...
      const fixtureTheme = (themesJson.data || []).find(
        (theme: any) => theme?.id === fixture.themeId,
      );
      expect(fixtureTheme).toBeTruthy();
      const fixtureThemeVersionId = String(
        fixtureTheme?.versions?.[0]?.id || "",
      );
      ...
      const createStoreRes = await fetch(`${baseUrl}/api/stores`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: "Theme Components Fixture Store",
          autoSeed: true,
        }),
      });
      ...
      const bindStoreThemeRes = await fetch(
      ...
      const aliasDropdown = fixtureComponent?.fields?.find(
        (field: any) => field?.id === "brand_selector_alias",
      );
      const inferredDropdown = fixtureComponent?.fields?.find(
        (field: any) => field?.id === "products_selector_inferred",
      );
      const aliasVariableList = fixtureComponent?.fields?.find(
        (field: any) => field?.id === "variable_sources_aliases",
      );
      const inferredVariableList = fixtureComponent?.fields?.find(
        (field: any) => field?.id === "variable_sources_default",
      );
      ...
      const aliasSources = (aliasVariableList?.variableSources || []).map(
        (entry: any) => entry?.value,
      );
      expect(aliasSources).toEqual(
        expect.arrayContaining(["products", "blog_articles", "brands"]),
      );
      ...
      expect(
        Object.prototype.hasOwnProperty.call(aliasOptions, "product"),
      ).toBe(false);
      expect(Object.prototype.hasOwnProperty.call(aliasOptions, "post")).toBe(
        false,
      );
      expect(Object.prototype.hasOwnProperty.call(aliasOptions, "brand")).toBe(
        false,
      );

      const inferredSources = (inferredVariableList?.variableSources || []).map(
        (entry: any) => entry?.value,
      );
      expect(inferredSources).toContain("blog_articles");
      expect(
        Array.isArray(inferredVariableList?.variableOptions?.blog_articles),
      ).toBe(true);
    } finally {
  ...
  it("allows binding a store to a failing theme and returns observe diagnostics", async () => {
    ...
    try {
      ...
      expect(
        String(bindStoreThemeJson.data?.themeAdmission?.overallStatus || ""),
      ).toBe("fail");
    } finally {
      fs.rmSync(fixture.themeDir, { recursive: true, force: true });
    }
  ...
  it("keeps theme matrix gate observable (pass/warning/fail) while allowing store binding", async () => {
    ...
    const fixtureDirs = [
      passFixture.themeDir,
      warningFixture.themeDir,
      failFixture.themeDir,
    ];
    ...
    try {
      ...
      const capabilityGateThemes = Array.isArray(
      ...
      expect(String(syncJson?.data?.capabilityGate?.overallStatus || "")).toBe(
        "fail",
      );
      ...
      const passThemeVersionId = String(
        themes.find((theme: any) => theme?.id === passFixture.themeId)
          ?.versions?.[0]?.id || "",
      );
      const warningThemeVersionId = String(
        themes.find((theme: any) => theme?.id === warningFixture.themeId)
          ?.versions?.[0]?.id || "",
      );
      const failThemeVersionId = String(
        themes.find((theme: any) => theme?.id === failFixture.themeId)
          ?.versions?.[0]?.id || "",
      );
      ...
      expect(String(bindFailJson.data?.themeAdmission?.mode || "")).toBe(
        "observe",
      );
      expect(
        String(bindFailJson.data?.themeAdmission?.overallStatus || ""),
      ).toBe("fail");
    } finally {
  ...
  it("isolates preview scope across stores and exposes render baseline headers", async () => {
    ...
    const totalMsA = Number(
      previewA.headers.get("x-vtdr-render-total-ms") || "NaN",
    );
    const contextMsA = Number(
      previewA.headers.get("x-vtdr-render-context-ms") || "NaN",
    );
    const p95MsB = Number(
      previewB.headers.get("x-vtdr-render-p95-ms") || "NaN",
    );
  ...
  it("keeps preview isolation under concurrent multi-store requests and exposes system metrics snapshot", async () => {
    ...
    for (const entry of results) {
      ...
      const totalMs = Number(
        entry.response.headers.get("x-vtdr-render-total-ms") || "NaN",
      );
      const p95Ms = Number(
        entry.response.headers.get("x-vtdr-render-p95-ms") || "NaN",
      );
    ...
    const metricsRes = await fetch(
      `${baseUrl}/api/system/preview/metrics?limit=50`,
    );
    ...
    const metricStoreIds = new Set(
      (metricsJson.data.metrics || []).map((entry: any) =>
  ...
  it("covers store lifecycle operations clone/promote/inherit/seed/sync validations", async () => {
    ...
    const promoteRes = await fetch(
      `${baseUrl}/api/stores/${parentStoreId}/promote`,
      {
        method: "POST",
      },
    );
    ...
    const inheritRes = await fetch(
      `${baseUrl}/api/stores/${childStoreId}/inherit`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const cloneRes = await fetch(
      `${baseUrl}/api/stores/${parentStoreId}/clone`,
      {
        method: "POST",
      },
    );
    ...
    const syncMissingUrlRes = await fetch(
      `${baseUrl}/api/stores/${childStoreId}/sync`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
    ...
    const syncInvalidUrlRes = await fetch(
      `${baseUrl}/api/stores/${childStoreId}/sync`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  it("supports inventory stock filters/sorting and enforces cart stock limits", async () => {
    ...
    const inStockListRes = await fetch(
      `${baseUrl}/api/v1/products?status=in-stock`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(
      inStockListJson.data.some((entry: any) => entry.id === "inv-in-stock"),
    ).toBe(true);
    expect(
      inStockListJson.data.some((entry: any) => entry.id === "inv-mid-stock"),
    ).toBe(true);
    expect(
      inStockListJson.data.some((entry: any) => entry.id === "inv-out-stock"),
    ).toBe(false);
    expect(
      inStockListJson.data.some((entry: any) => entry.id === "inv-backorder"),
    ).toBe(false);

    const outStockListRes = await fetch(
      `${baseUrl}/api/v1/products?status=out-of-stock`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(
      outStockListJson.data.some((entry: any) => entry.id === "inv-out-stock"),
    ).toBe(true);
    expect(
      outStockListJson.data.some((entry: any) => entry.id === "inv-in-stock"),
    ).toBe(false);
    expect(
      outStockListJson.data.some((entry: any) => entry.id === "inv-backorder"),
    ).toBe(false);

    const lowStockListRes = await fetch(
      `${baseUrl}/api/v1/products?status=low-stock`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(
      lowStockListJson.data.some((entry: any) => entry.id === "inv-mid-stock"),
    ).toBe(true);
    expect(
      lowStockListJson.data.some((entry: any) => entry.id === "inv-in-stock"),
    ).toBe(false);

    const backorderListRes = await fetch(
      `${baseUrl}/api/v1/products?status=backorder`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(
      backorderListJson.data.some((entry: any) => entry.id === "inv-backorder"),
    ).toBe(true);
    expect(
      backorderListJson.data.some((entry: any) => entry.id === "inv-out-stock"),
    ).toBe(false);

    const sortedRes = await fetch(
      `${baseUrl}/api/v1/products?sort=priceFromLowToTop`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    const sortedIds = (sortedJson.data || []).map((entry: any) =>
      String(entry.id),
    );
    expect(sortedIds.slice(0, 3)).toEqual([
      "inv-out-stock",
      "inv-backorder",
      "inv-mid-stock",
    ]);

    const sortedByStockRes = await fetch(
      `${baseUrl}/api/v1/products?sort=stockFromLowToTop`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    const sortedByStockIds = (sortedByStockJson.data || []).map((entry: any) =>
      String(entry.id),
    );
    expect(sortedByStockIds.slice(0, 2).sort()).toEqual(
      ["inv-out-stock", "inv-backorder"].sort(),
    );

    const featuredSortedRes = await fetch(
      `${baseUrl}/api/v1/products?sort=featured`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(
      String(
        addBeyondLimitJson.error?.message || addBeyondLimitJson.error || "",
      ),
    ).toContain("Maximum available quantity");

    const updateBeyondLimitRes = await fetch(
      `${baseUrl}/api/v1/cart/items/inv-in-stock`,
      {
        method: "PATCH",
        headers: contextHeaders,
        body: JSON.stringify({
    ...
    const updateWithinLimitRes = await fetch(
      `${baseUrl}/api/v1/cart/items/inv-in-stock`,
      {
        method: "PATCH",
        headers: contextHeaders,
  ...
  it("supports menus read/write with nested items for preview navigation", async () => {
    ...
    expect(
      putHeaderJson.data.some((item: any) => item.id === "header-main"),
    ).toBe(true);
    ...
    const headerMain = (getHeaderJson.data || []).find(
      (item: any) => item.id === "header-main",
    );
  ...
  it("supports reviews/questions CRUD and updates product feedback metrics", async () => {
    ...
    const productAfterReviewsRes = await fetch(
      `${baseUrl}/api/v1/products/product-feedback`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    const productAfterReviewUpdateRes = await fetch(
      `${baseUrl}/api/v1/products/product-feedback`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    const productAfterReviewUpdateJson: any =
      await productAfterReviewUpdateRes.json();
    ...
    const updateQuestionRes = await fetch(
      `${baseUrl}/api/v1/questions/question-1`,
      {
        method: "PUT",
        headers: contextHeaders,
        body: JSON.stringify({
    ...
    const questionsByProductRes = await fetch(
      `${baseUrl}/api/v1/products/product-feedback/questions`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    const productAfterQuestionRes = await fetch(
      `${baseUrl}/api/v1/products/product-feedback`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
  ...
  it("supports brands/offers CRUD and propagates brand options into theme components", async () => {
    ...
    expect(
      listBrandsJson.data.some((entry: any) => entry.id === "brand-vtdr"),
    ).toBe(true);
    ...
    expect(Array.isArray(brandsField?.options)).toBe(true);
    expect(
      brandsField.options.some((opt: any) => opt.value === "brand-vtdr"),
    ).toBe(true);
    ...
    expect(
      listOffersJson.data.some((entry: any) => entry.id === "offer-vtdr"),
    ).toBe(true);
    ...
    const themeComponentsAfterDeleteRes = await fetch(
      `${baseUrl}/api/v1/theme/components`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    const themeComponentsAfterDeleteJson: any =
      await themeComponentsAfterDeleteRes.json();
    const homeBrandsAfterDelete = (
      themeComponentsAfterDeleteJson.data.components || []
    ).find((c: any) => c.path === "home.brands");
    const brandsFieldAfterDelete = homeBrandsAfterDelete?.fields?.find(
      (f: any) => f.id === "brands",
    );
    expect(Array.isArray(brandsFieldAfterDelete?.options)).toBe(true);
    expect(
      brandsFieldAfterDelete.options.some(
        (opt: any) => opt.value === "brand-vtdr",
      ),
    ).toBe(false);
    ...
    expect(
      offersAfterDeleteJson.data.some(
        (entry: any) => entry.id === "offer-vtdr",
      ),
    ).toBe(false);
  });
  ...
  it("supports blog content CRUD and propagates to theme variable-list options", async () => {
    ...
    expect(
      listArticlesJson.data.some(
        (entry: any) => entry.id === "blog-article-vtdr",
      ),
    ).toBe(true);
    ...
    const articleOptions = variableListFields.flatMap((field: any) =>
      Array.isArray(field?.variableOptions?.blog_articles)
        ? field.variableOptions.blog_articles
        : [],
    );
    const categoryOptions = variableListFields.flatMap((field: any) =>
      Array.isArray(field?.variableOptions?.blog_categories)
        ? field.variableOptions.blog_categories
        : [],
    );
    expect(
      articleOptions.some((opt: any) => opt.value === "blog-article-vtdr"),
    ).toBe(true);
    expect(
      categoryOptions.some((opt: any) => opt.value === "blog-cat-tech"),
    ).toBe(true);

    const updateArticleRes = await fetch(
      `${baseUrl}/api/v1/blog/articles/blog-article-vtdr`,
      {
        method: "PUT",
        headers: contextHeaders,
        body: JSON.stringify({
    ...
    const getUpdatedArticleRes = await fetch(
      `${baseUrl}/api/v1/blog/articles/blog-article-vtdr`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    const deleteCategoryRes = await fetch(
      `${baseUrl}/api/v1/blog/categories/blog-cat-tech`,
      {
        method: "DELETE",
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    const getArticleAfterCategoryDeleteRes = await fetch(
      `${baseUrl}/api/v1/blog/articles/blog-article-vtdr`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    const getArticleAfterCategoryDeleteJson: any =
      await getArticleAfterCategoryDeleteRes.json();
    expect(
      String(getArticleAfterCategoryDeleteJson.data.category_id || ""),
    ).toBe("");
  });
  ...
  it("normalizes product/category parity contracts and keeps preview functional", async () => {
    ...
    expect(categoriesRes.status).toBe(200);
    const categoriesJson: any = await categoriesRes.json();
    const rootCategory = categoriesJson.data.find(
      (c: any) => c.id === "cat-root",
    );
    const childCategory = categoriesJson.data.find(
      (c: any) => c.id === "cat-child",
    );
    ...
    const getProductRes = await fetch(
      `${baseUrl}/api/v1/products/prod-parity-1`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(product.categories.some((c: any) => c.id === "cat-child")).toBe(
      true,
    );
    ...
    const updateProductRes = await fetch(
      `${baseUrl}/api/v1/products/prod-parity-1`,
      {
        method: "PUT",
        headers: contextHeaders,
    ...
    const getUpdatedProductRes = await fetch(
      `${baseUrl}/api/v1/products/prod-parity-1`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    expect(
      updatedProduct.categories.some((c: any) => c.id === "cat-root"),
    ).toBe(true);
    ...
    const deleteCategoryRes = await fetch(
      `${baseUrl}/api/v1/categories/cat-root`,
      {
        method: "DELETE",
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    const categoriesAfterDeleteRes = await fetch(
      `${baseUrl}/api/v1/categories`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    const categoriesAfterDeleteJson: any =
      await categoriesAfterDeleteRes.json();
    expect(
      categoriesAfterDeleteJson.data.some((c: any) => c.id === "cat-root"),
    ).toBe(false);
    const childAfterDelete = categoriesAfterDeleteJson.data.find(
      (c: any) => c.id === "cat-child",
    );
    ...
    const productAfterDeleteRes = await fetch(
      `${baseUrl}/api/v1/products/prod-parity-1`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    expect(productAfterDeleteJson.data.category_ids.includes("cat-root")).toBe(
      false,
    );
  ...
  it("supports customer journey parity for wishlist/orders pages and APIs", async () => {
    ...
    expect(addWishlistJson.data.product_ids).toContain(
      "wishlist-preview-product",
    );
    ...
    expect(getWishlistJson.data.product_ids).toContain(
      "wishlist-preview-product",
    );

    const wishlistProductsRes = await fetch(
      `${baseUrl}/api/v1/products?source=wishlist`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(
      wishlistProductsJson.data.some(
        (entry: any) => entry.id === "wishlist-preview-product",
      ),
    ).toBe(true);
    ...
    const wishlistProductsAfterToggleRes = await fetch(
      `${baseUrl}/api/v1/products?source=wishlist`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    ...
    const wishlistProductsAfterToggleJson: any =
      await wishlistProductsAfterToggleRes.json();
    expect(
      wishlistProductsAfterToggleJson.data.some(
        (entry: any) => entry.id === "wishlist-preview-product",
      ),
    ).toBe(false);
    ...
    const addressCheckoutRes = await fetch(
      `${baseUrl}/api/v1/checkout/address`,
      {
        method: "PATCH",
        headers: contextHeaders,
        body: JSON.stringify({
    ...
    const shippingMethodId = String(
      checkoutDetailsJson.data.available_shipping_methods?.[0]?.id || "",
    );
    const paymentMethodId = String(
      checkoutDetailsJson.data.available_payment_methods?.[0]?.id || "",
    );
    expect(shippingMethodId).toBeTruthy();
    expect(paymentMethodId).toBeTruthy();

    const shippingCheckoutRes = await fetch(
      `${baseUrl}/api/v1/checkout/shipping`,
      {
        method: "PATCH",
        headers: contextHeaders,
    ...
    const paymentCheckoutRes = await fetch(
      `${baseUrl}/api/v1/checkout/payment`,
      {
        method: "PATCH",
        headers: contextHeaders,
    ...
    const confirmCheckoutRes = await fetch(
      `${baseUrl}/api/v1/checkout/confirm`,
      {
        method: "POST",
        headers: contextHeaders,
        body: JSON.stringify({}),
    ...
    expect(
      listOrdersJson.data.some((entry: any) => String(entry.id) === orderId),
    ).toBe(true);
    ...
    expect(
      customerOrdersPreviewRes.status,
      customerOrdersPreviewHtml.slice(0, 600),
    ).toBe(200);
    ...
    const customerSingleOrderPreviewHtml =
      await customerSingleOrderPreviewRes.text();
    expect(
      customerSingleOrderPreviewRes.status,
      customerSingleOrderPreviewHtml.slice(0, 600),
    ).toBe(200);
  ...
  it("supports related products source and enriched offers payload contracts", async () => {
    ...
    const createStoreRes = await fetch(`${baseUrl}/api/stores`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        title: "Related And Offers Contract Store",
        autoSeed: false,
      }),
    });
    expect(createStoreRes.status).toBe(200);
    ...
    const relatedIds = relatedProductsJson.data.map((entry: any) =>
      String(entry?.id || ""),
    );
    ...
    const getOfferRes = await fetch(
      `${baseUrl}/api/v1/offers/offer-related-contract`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(getOfferJson.data.product_ids).toEqual(
      expect.arrayContaining(["related-strong", "related-medium"]),
    );
    expect(getOfferJson.data.category_ids).toContain("cat-related-main");
    expect(
      getOfferJson.data.products.map((entry: any) => String(entry?.id || "")),
    ).toEqual(expect.arrayContaining(["related-strong", "related-medium"]));
    expect(
      getOfferJson.data.categories.map((entry: any) => String(entry?.id || "")),
    ).toContain("cat-related-main");
  }, 120000);
  ...
  it("supports donation product contract via products API", async () => {
    ...
    const getDonationProductRes = await fetch(
      `${baseUrl}/api/v1/products/donation-contract-product`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
  ...
  it("sets donation.can_donate=false when campaign is expired or target reached", async () => {
    ...
    const createStoreRes = await fetch(`${baseUrl}/api/stores`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        title: "Donation Closed Campaign Store",
        autoSeed: false,
      }),
    });
    expect(createStoreRes.status).toBe(200);
    ...
    const getExpiredDonationRes = await fetch(
      `${baseUrl}/api/v1/products/donation-expired-product`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(Boolean(getExpiredDonationJson.data?.donation?.can_donate)).toBe(
      false,
    );
    expect(
      String(getExpiredDonationJson.data?.donation?.target_message || ""),
    ).toContain("انتهت");

    const getReachedDonationRes = await fetch(
      `${baseUrl}/api/v1/products/donation-reached-product`,
      {
        headers: { "X-VTDR-Store-Id": storeId },
      },
    );
    ...
    expect(Boolean(getReachedDonationJson.data?.donation?.can_donate)).toBe(
      false,
    );
    expect(
      Number(getReachedDonationJson.data?.donation?.target_percent || 0),
    ).toBe(100);
    expect(
      String(getReachedDonationJson.data?.donation?.target_message || ""),
    ).toContain("الوصول");
  }, 120000);
});
