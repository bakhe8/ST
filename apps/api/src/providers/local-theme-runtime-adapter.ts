...
export class LocalThemeRuntimeAdapter implements IThemeRuntimeAdapter {
    ...
    public async getThemeSchema(themeId: string): Promise<any | null> {
        ...
        const twilightPath = path.join(this.themesBaseDir, normalizedThemeId, 'twilight.json');
        ...
        try {
            ...
            console.error(`[LocalThemeRuntimeAdapter] Failed to parse twilight.json for ${normalizedThemeId}:`, error);
            return null;
        }
    }
    ...
    public resolvePlaceholderImage(themeId?: string | null): string {
        ...
        for (const candidate of candidates) {
            const absolute = path.join(this.themesBaseDir, normalizedThemeId, candidate);
