...
export async function bootstrapGuard(port: number) {
    ...
    if (isPortBusy) {
        ...
        console.error(`[BOOTSTRAP_HELP] This usually means a zombie Node.js process is running.`);
        console.error(`[BOOTSTRAP_HELP] Run: taskkill /F /IM node.exe /T to clear it.`);
        process.exit(1);
    }
    ...
    try {
        ...
        if (fs.existsSync(enginePath)) {
            ...
            console.warn(`[BOOTSTRAP] Engine Link Verified: ${pkg.name}@${pkg.version}`);
        }
    } catch (e) {
        console.error(`[BOOTSTRAP_WARNING] Could not verify Engine link. Build may be unstable.`);
    }
