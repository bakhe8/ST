...
export function createThemeRoutes(
  themeManagementOrchestrator: ThemeManagementOrchestrator,
) {
  const router = Router();
  ...
  router.post("/register", async (req, res) => {
    const result = await themeManagementOrchestrator.registerTheme(
      req.body?.folder,
    );
  ...
  router.get("/:themeId/changelog", async (req, res) => {
    const result = await themeManagementOrchestrator.getThemeChangeLog(
      req.params.themeId,
    );
    if (!result.ok) return fail(res, result.status, result.message);
    return ok(res, result.data);
