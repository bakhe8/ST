...
export function createRuntimeRoutes(
    ...
    const isPreviewTranslationScriptRequest = (assetName: string, storeId: string) => {
...
`;
    ...
    const renderPreview = async (req: StoreRequest, res: Response, wildcardPath = '') => {
        try {
            ...
            res.setHeader('X-VTDR-Render-Context-Ms', String(result.metrics.contextBuildMs));
    ...
    router.get('/preview/:storeId/:themeId/:version/*rest', async (req: StoreRequest, res: Response) => {
        const params = req.params as Record<string, unknown>;
        const wildcardPath = resolveWildcardPath(params.rest);
        if (isPreviewTranslationScriptRequest(wildcardPath, String(params.storeId || ''))) {
            const scriptBase = extractJsBaseName(wildcardPath);
            return res
                .status(200)
                .type('application/javascript')
                .send(buildTranslationStubScript(String(params.storeId || ''), scriptBase));
        }
    ...
    router.post('/render', async (req: StoreRequest, res: Response) => {
        const result = await previewOrchestrator.buildStoreContext(req.storeId || '');
        if (!result.ok) {
