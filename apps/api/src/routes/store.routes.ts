...
export function createStoreRoutes(
  ...
  router.patch("/:id", mw, async (req: StoreRequest, res: Response) => {
    try {
      const store = await storeManagementOrchestrator.updateStore(
        req.storeId!,
        req.body,
      );
      return ok(res, store);
  ...
  router.post("/:id/inherit", mw, async (req: StoreRequest, res: Response) => {
    const result = await storeManagementOrchestrator.inheritStore(
      req.storeId!,
      req.body?.parentStoreId,
    );
    return sendStoreResult(res, result);
  });
  ...
  router.patch(
    ...
    async (req: StoreRequest, res: Response) => {
      const result = await storeManagementOrchestrator.updateSettings(
        req.storeId!,
        req.body?.settings,
      );
      return sendStoreResult(res, result);
  ...
  router.post("/:id/sync", mw, async (req: StoreRequest, res: Response) => {
    const result = await storeManagementOrchestrator.syncStore(
      req.storeId!,
      req.body?.storeUrl,
    );
    return sendStoreResult(res, result);
  ...
  router.delete("/:id/data", mw, async (req: StoreRequest, res: Response) => {
    const result = await storeManagementOrchestrator.clearStoreData(
      req.storeId!,
    );
    return sendStoreResult(res, result);
  });
