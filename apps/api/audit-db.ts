...
async function runAudit() {
    ...
    console.log(`📂 Connecting to: ${process.env.DATABASE_URL || 'Default from Schema'}\n`);
    ...
    try {
        ...
        for (const store of stores) {
            ...
            try {
                ...
                results.invalidJson++;
                results.errors.push(`[Store ${store.id}] Invalid brandingJson: ${e.message}`);
            ...
            try {
                ...
                results.invalidJson++;
                results.errors.push(`[Store ${store.id}] Invalid settingsJson: ${e.message}`);
        ...
        const orphanEntities = dataEntities.filter(de => !storeIds.has(de.storeId));

        if (orphanEntities.length > 0) {
            results.orphans += orphanEntities.length;
            results.errors.push(`Found ${orphanEntities.length} orphan DataEntities.`);
