...
(function () {
  ...
  const parseSelected = (raw) => {
    ...
    return value.split(",").map((x) => str(x)).filter(Boolean);
  };
  ...
  const callApi = async (path, query = {}) => {
    ...
    if (!res.ok || payload?.success === false) {
      const msg = payload?.error?.message || payload?.error || payload?.message || res.statusText || "API error";
      throw new Error(msg);
    }
    return payload || {};
  };
  ...
  const normalizeProduct = (p) => {
    ...
    const imageUrl = str(x?.image?.url || x.thumbnail || x.main_image || "/images/placeholder.png");
    const amount = Number(x?.price?.amount ?? x?.sale_price?.amount ?? x?.price ?? 0) || 0;
    ...
    return {
      ...
      image: { ...(x.image || {}), url: imageUrl, alt: str(x?.image?.alt || x.name || "Product") },
      price: x?.price && typeof x.price === "object" ? x.price : { amount, currency },
    };
  };
  ...
  const ensureComponentApi = () => {
    ...
    salla.api.component.getProducts = async (opts = {}) => {
      ...
      if (source === "selected") {
        ...
        const ids = new Set(parseSelected(sourceValue).map((x) => x.toLowerCase()));
        const base = await callApi("/api/v1/products", query);
        const items = arr(base.data)
          .map(normalizeProduct)
          .filter((p) => ids.size === 0 || ids.has(str(p.id).toLowerCase()) || ids.has(str(p.slug).toLowerCase()) || ids.has(tail(p.url).toLowerCase()));
        return {
          ...base,
          data: items,
          pagination: { ...(base.pagination || {}), count: items.length, total: items.length, currentPage: 1, totalPages: 1 },
        };
      }
    ...
    salla.api.component.getMenus = async (opts = {}) => {
      const source = typeof opts === "string" ? str(opts) : str(opts.source || opts.type || "header") || "header";
      return callApi(`/api/v1/menus/${encodeURIComponent(source)}`);
    };

    salla.api.component.getCategories = async (opts = {}) => callApi("/api/v1/categories", opts.query || {});
    salla.api.component.getBrands = async (opts = {}) => callApi("/api/v1/brands", opts.query || {});
  ...
  class VtdrProductsList extends HTMLElement {
    static get observedAttributes() { return ["source", "source-value", "filters"]; }
    constructor() { super(); this.sortBy = ""; this.__connected = false; this.__onFilters = this.onFilters.bind(this); }
    connectedCallback() {
    ...
    disconnectedCallback() { document.removeEventListener("salla-filters::changed", this.__onFilters); }
    attributeChangedCallback(a, oldValue, newValue) { if (!this.__connected || oldValue === newValue) return; this.reload(); }
    onFilters(e) {
      try { this.setAttribute("filters", JSON.stringify(e?.detail || {})); } catch {}
      this.reload();
    }
    parseFilters() {
    ...
    createCard(product) {
      const tag = str(this.getAttribute("product-card-component") || "custom-salla-product-card");
      if (tag.includes("-")) {
        const el = document.createElement(tag);
        el.setAttribute("product", JSON.stringify(product));
        if (this.hasAttribute("row-cards")) el.setAttribute("horizontal", "");
        if (this.hasAttribute("shadow-on-hover")) el.setAttribute("shadowOnHover", "");
        return el;
      }
      const a = document.createElement("a");
      a.className = "vtdr-card";
      a.href = product.url || "#";
      const money = window.salla?.money ? window.salla.money(product?.price?.amount || 0) : `${product?.price?.amount || 0}`;
    ...
    async reload() {
      ...
      const sourceValue = str(this.getAttribute("source-value") || this.getAttribute("source_value"));
      ...
      try {
        ensureComponentApi();
        const res = await window.salla.api.component.getProducts({ source, sourceValue, sort, query });
        const items = arr(res.data).map(normalizeProduct);
        if (!items.length) {
          this.innerHTML = `<div class="vtdr-empty">No products</div>`;
        } else {
          const wrap = document.createElement("div");
          wrap.className = this.hasAttribute("row-cards") ? "vtdr-list" : "vtdr-grid";
          items.forEach((p) => { const w = document.createElement("div"); w.appendChild(this.createCard(p)); wrap.appendChild(w); });
          this.innerHTML = "";
          this.appendChild(wrap);
        }
        const payload = { source, source_value: sourceValue, title: str(res.title), data: items, pagination: res.pagination || {} };
        if (window.salla?.event?.dispatch) window.salla.event.dispatch("salla-products-list::products.fetched", payload);
        return res;
      } catch (e) {
  ...
  class VtdrProductsSlider extends HTMLElement {
    connectedCallback() {
      ...
      if (this.hasAttribute("product-card-component")) list.setAttribute("product-card-component", this.getAttribute("product-card-component"));
      if (this.hasAttribute("row-cards")) list.setAttribute("row-cards", "");
      if (this.hasAttribute("shadow-on-hover")) list.setAttribute("shadow-on-hover", "");
      this.innerHTML = "";
  ...
  class VtdrMenu extends HTMLElement {
    connectedCallback() { if (this.__connected) return; this.__connected = true; ensureBaseStyle(); this.reload(); }
    async reload() {
      try {
        ensureComponentApi();
  ...
  try {
    ensureComponentApi();
    if (!customElements.get("salla-products-list")) customElements.define("salla-products-list", VtdrProductsList);
    if (!customElements.get("salla-products-slider")) customElements.define("salla-products-slider", VtdrProductsSlider);
    if (!customElements.get("salla-menu")) customElements.define("salla-menu", VtdrMenu);
  } catch (e) {
