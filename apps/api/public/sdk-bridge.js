...
(function () {
  ...
  const isStoreLikeDomain = (hostname) => {
    ...
    return (
      /(^|\.)salla\.sa$/i.test(hostname) || /(^|\.)demo\.sa$/i.test(hostname)
    );
  };
  ...
  const writeWishlistIds = (ids) => {
    const normalized = Array.from(
      new Set((ids || []).map((entry) => String(entry).trim()).filter(Boolean)),
    );
    try {
  ...
  const emitWishlistRemoved = (productId, payload) => {
    ...
    if (salla.event?.dispatch) {
      salla.event.dispatch("wishlist::removed", {
        id: productId,
        data: payload,
      });
    }
  ...
  const emitCartItemUpdateFailed = (error, itemId) => {
    cartItemUpdatedFailedHandlers.forEach((handler) => {
      try {
        ...
        console.error(
          "[VTDR Bridge] cart item update failed handler error",
          err,
        );
      }
    });
  ...
  salla.cart.event.onItemUpdatedFailed = (handler) => {
    if (typeof handler === "function")
      cartItemUpdatedFailedHandlers.push(handler);
  };
  ...
  const normalizeCartItemPayload = (payload) => {
    ...
    const price = toNumber(
      input.price ?? input.unit_price ?? input.sale_price,
      0,
    );
  ...
  const cartProxy = {
    ...
    deleteItem: async (itemId) => {
      ...
      const result = await callLocalApi(
        `/api/v1/cart/items/${encodeURIComponent(normalizedId)}`,
        "DELETE",
      );
      if (result?.data) {
        emitCartUpdated(result.data);
        if (salla.event?.dispatch)
          salla.event.dispatch("cart::item-removed", result);
      }
      return result;
    },
    addCoupon: async (couponCode) => {
      const result = await callLocalApi("/api/v1/cart/coupon", "POST", {
  ...
  const checkoutProxy = {
    ...
    start: async (payload) => {
      ...
      const step = String(result?.data?.step || "address")
        .trim()
        .toLowerCase();
    ...
    updateAddress: async (payload) => {
      ...
      const step = String(result?.data?.step || "shipping")
        .trim()
        .toLowerCase();
      navigateInPreview("/checkout", step ? { step } : null);
      return result;
    ...
    updatePayment: async (payload) => {
      ...
      const step = String(result?.data?.step || "review")
        .trim()
        .toLowerCase();
      navigateInPreview("/checkout", step ? { step } : null);
      return result;
  ...
  const persistWishlistFromApiResponse = (result, fallbackIds = []) => {
    const apiIds = Array.isArray(result?.data?.product_ids)
      ? result.data.product_ids
      : fallbackIds;
  ...
  const wishlistProxy = {
    ...
    add: async (productId) => {
      ...
      const ids = persistWishlistFromApiResponse(result, [
        ...readWishlistIds(),
        payload.product_id,
      ]);
    ...
    remove: async (productId) => {
      ...
      const result = await callLocalApi(
        `/api/v1/wishlist/items/${encodeURIComponent(targetId)}`,
        "DELETE",
      );
      const ids = persistWishlistFromApiResponse(
        result,
        readWishlistIds().filter((id) => id !== targetId),
      );
    ...
    toggle: async (productId) => {
      ...
      const action = String(result?.data?.action || "")
        .trim()
        .toLowerCase();
      if (action === "removed") {
  ...
  const orderProxy = {
    details: async (orderId) => {
      ...
      return callLocalApi(
        `/api/v1/orders/${encodeURIComponent(targetId)}`,
        "GET",
      );
    },
    list: async (query = {}) => {
  ...
  const originalOnSubmit =
    typeof originalForm.onSubmit === "function"
      ? originalForm.onSubmit.bind(originalForm)
      : null;
  const originalOnChange =
    typeof originalForm.onChange === "function"
      ? originalForm.onChange.bind(originalForm)
      : null;
    ...
    const run = async () => {
      ...
      if (normalizedAction === "order.sendInvoice") {
        ...
        const orderId = String(
          formPayload.order_id || currentUrl.searchParams.get("order_id") || "",
        ).trim();
        const payload = {
          ...formPayload,
  ...
  salla.form.onChange = function (action, event) {
    const run = async () => {
      ...
      if (normalizedAction === "cart.updateItem") {
        ...
        const itemId = String(
          payload.id || payload.item_id || payload.product_id || "",
        ).trim();
  ...
  document.addEventListener(
    ...
    (event) => {
      const toggleBtn = event.target?.closest?.(
        ".s-product-card-wishlist-btn[data-id], .btn--wishlist[data-id]",
      );
      if (toggleBtn) {
      ...
      if (removeBtn) {
        ...
        const productId = String(wishlistItem?.id || "")
          .replace(/^wishlist-product-/, "")
          .trim();
        if (!productId) return;
  ...
  document.addEventListener("click", (event) => {
    ...
    const externalStoreLike =
      !sameOrigin &&
      (isStoreLikeDomain(url.hostname) || isLocalDevHost(url.hostname));
