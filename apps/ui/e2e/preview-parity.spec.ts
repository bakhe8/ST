...
function hasTemplatePageMarker(
  html: string,
...
async function fetchEntityCounts(
  ...
  const [productsJson, categoriesJson, brandsJson, blogArticlesJson] =
    await Promise.all([
      productsRes.json(),
      categoriesRes.json(),
      brandsRes.json(),
...
async function createPreviewFixture(request: any): Promise<PreviewFixture> {
  ...
  const themeVersion = String(
    storePayload?.data?.themeVersion?.version || "",
  ).trim();
...
test.describe("VTDR Preview Browser Parity Gate", () => {
  test("renders preview routes with semantic parity markers (core + extended storefront)", async ({
    page,
    request,
  }) => {
    const { storeId, themeId, themeVersion, counts } =
      await createPreviewFixture(request);
    ...
    page.on("console", (message) => {
      if (message.type() === "error") {
        consoleErrors.push(
          `[${themeId}][${activeRoutePath}] ${message.text()}`,
        );
      }
    });
    ...
    for (const routePath of PREVIEW_PATHS) {
      ...
      if (response.status() >= 500) {
        pushRouteFailure(
          routePath,
          `response status is ${response.status()} (expected < 500)`,
        );
        continue;
      }
      ...
      if (
        html.includes("Renderer Error") ||
        html.includes("Preview rendering failed")
      ) {
        pushRouteFailure(
          routePath,
          "rendered document contains renderer error marker",
        );
      }
      if (routePath !== "/" && html.includes("content--single-page")) {
        pushRouteFailure(
          routePath,
          "route resolved to page-single fallback template",
        );
      }
      if (routePath !== "/" && html.includes('item-id="=desktop&refresh=0"')) {
        pushRouteFailure(
          routePath,
          "route emitted malformed page id marker (=desktop&refresh=0)",
        );
      }
      ...
      if (routePath === "/products") {
        const productsListCount = await page
          .locator("salla-products-list")
          .count();
        if (productsListCount === 0) {
          pushRouteFailure(
            routePath,
            "missing <salla-products-list> component",
          );
        }
        if (counts.products > 0) {
          ...
          try {
            ...
            pushRouteFailure(
              routePath,
              "products API has data but no visible product cards were rendered",
            );
          }
        }
        if (bodyText.includes("pages.categories.sorting")) {
          pushRouteFailure(
            routePath,
            "raw translation key leaked: pages.categories.sorting",
          );
        }
      }
      ...
      if (routePath === "/categories") {
        ...
        if (productsListCount === 0) {
          pushRouteFailure(
            routePath,
            "missing category/product list runtime component",
          );
        }
        if (counts.categories === 0) {
          pushRouteFailure(
            routePath,
            "categories API returned no entities for categories page parity",
          );
        }
        if (bodyText.includes("pages.categories.sorting")) {
          pushRouteFailure(
      ...
      if (routePath === "/brands") {
        if (counts.brands === 0) {
          pushRouteFailure(
            routePath,
            "brands API returned no entities for brands page parity",
          );
        }
        if (counts.brands > 0) {
          ...
          try {
            ...
            pushRouteFailure(
              routePath,
              "brands API has data but no visible .brand-item was rendered",
            );
          }
        }
        if (bodyText.includes("pages.brands.non_brands")) {
          pushRouteFailure(
            routePath,
            "raw translation key leaked: pages.brands.non_brands",
          );
        }
      }
      ...
      if (routePath === "/blog") {
        ...
        if (counts.blogArticles > 0) {
          ...
          const blogCategoryLinks = page.locator(
            '#filters-menu a[href*="/blog"], .dropdown__menu a[href*="/blog"]',
          );
        ...
        if (bodyText.includes("pages.blog_categories.categories")) {
          pushRouteFailure(
            routePath,
            "raw translation key leaked: pages.blog_categories.categories",
          );
        }
        if (bodyText.includes("pages.blog_categories.no_articles")) {
          pushRouteFailure(
            routePath,
            "raw translation key leaked: pages.blog_categories.no_articles",
          );
        }
      }
    }
    ...
    const routesPassed = routeMetrics.filter(
      (item) => item.status === "pass",
    ).length;
    const routesFailed = routeMetrics.filter(
    ...
    expect(
      ...
      routeFailures.length
        ? `Preview semantic parity failures:\n${routeFailures.join("\n")}`
        : "all routes passed",
    ).toEqual([]);
    expect(
      criticalConsoleErrors,
