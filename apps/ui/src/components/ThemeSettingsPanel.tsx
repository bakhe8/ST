...
const ThemeSettingsPanel: React.FC<ThemeSettingsPanelProps> = ({ onSave }) => {
    ...
    fetch(apiUrl("v1/theme/settings"), {
      ...
      .then((response) => {
        if (response.success && response.data) {
          ...
          const activeFields = (settings || []).filter(
            (f: any) => f.type !== "static",
          );
  ...
  if (loading)
    return <div className="glass-card">جاري تحميل خيارات الثيم...</div>;

  if (fields.length === 0)
    return <div className="glass-card">لا توجد إعدادات لهذا الثيم.</div>;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {fields.map((field) => (
          ...
          >
            ...
            {field.description && (
              <div
                style={{ marginTop: 4, color: "#94a3b8", fontSize: "0.8rem" }}
              >
                {field.description}
              </div>
            )}
