...
const SystemHome = () => {
  ...
  const [themeSyncGate, setThemeSyncGate] =
    useState<ThemeCapabilityGate | null>(null);
  ...
  const mapCorePageName = (pageId: string) =>
    CORE_PAGE_LABELS[pageId] || pageId;
  const sortedThemeSnapshots = useMemo(() => {
    ...
    return [...themeSyncGate.themes].sort((a, b) => {
      ...
      const missingDelta =
        b.capability.missingCorePages.length -
        a.capability.missingCorePages.length;
      if (missingDelta !== 0) return missingDelta;

      const coverageA =
        a.capability.summary.coveredCorePages /
        Math.max(1, a.capability.summary.corePages);
      const coverageB =
        b.capability.summary.coveredCorePages /
        Math.max(1, b.capability.summary.corePages);
  ...
  const gateSummary = useMemo(() => {
    ...
    return {
      fail: source.filter(
        (theme) =>
          (theme.overallStatus || theme.capability.overallStatus) === "fail",
      ).length,
      warning: source.filter(
        (theme) =>
          (theme.overallStatus || theme.capability.overallStatus) === "warning",
      ).length,
      pass: source.filter(
        (theme) =>
          (theme.overallStatus || theme.capability.overallStatus) === "pass",
      ).length,
    };
  ...
  return (
    <div>
      ...
      >
        <div>
          ...
          <p style={{ color: "#94a3b8" }}>
            Manage and simulate your Salla stores.
          </p>
        </div>
        <button
      ...
      {isCreating && (
        ...
        >
          <h3 style={{ marginTop: 0, marginBottom: 16, color: "white" }}>
            New Store Details
          </h3>
          <div style={{ display: "flex", gap: 12 }}>
            ...
            <select
              value={seedProfile}
              onChange={(event) =>
                setSeedProfile(event.target.value as SeedProfile)
              }
              style={{
                padding: 12,
                borderRadius: 8,
        ...
        >
          <div>
            ...
            <p style={{ margin: "6px 0 0", color: "#94a3b8", fontSize: 13 }}>
              يتحقق من تغطية الصفحات الأساسية من `twilight.json` + فحص نقاط
              العرض (Anchor Probe) لكل ثيم بعد المزامنة.
            </p>
          </div>
          <button
          ...
          >
            <RefreshCw
              size={16}
              style={{
                animation: isSyncingThemes ? "spin 1s linear infinite" : "none",
              }}
            />
        ...
        {themeSyncGate && (
          <div>
            ...
            >
              ...
              <span
                style={{ color: "#94a3b8", fontSize: 12, padding: "3px 2px" }}
              >
            ...
            >
              {sortedThemeSnapshots.map((theme) => {
                const statusStyle = getStatusColors(
                  theme.overallStatus || theme.capability.overallStatus,
                );
                ...
                return (
                  ...
                  >
                    ...
                    >
                      Core coverage: {theme.capability.summary.coveredCorePages}
                      /{theme.capability.summary.corePages}
                    ...
                    {anchorProbe && (
                      ...
                      >
                        Anchor probe: {anchorProbe.summary.renderedComponents}/
                        {anchorProbe.summary.declaredComponents}
                        <span style={{ marginInlineStart: 8 }}>
                          Missing anchors:{" "}
                          {anchorProbe.summary.missingAnchorPoints}
                        </span>
                      </div>
                    )}
                    ...
                    {theme.capability.missingCorePages.length > 0 ? (
                      <div style={{ color: "#fbbf24", fontSize: 12 }}>
                        Missing core pages:{" "}
                        {theme.capability.missingCorePages
                          .map(mapCorePageName)
                          .join("، ")}
                      </div>
                    ) : (
                      <div style={{ color: "#34d399", fontSize: 12 }}>
                        Core storefront pages are covered.
                      </div>
                    )}
                  </div>
                );
