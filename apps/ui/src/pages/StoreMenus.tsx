...
const StoreMenus = () => {
  ...
  const addRootItem = () => {
    setItems((prev) => [
      ...prev,
      createMenuItem(`root-${menuType}`, prev.length + 1),
    ]);
  };
  ...
  const addChildItem = (parentId: string) => {
    setItems((prev) =>
      mapTree(prev, parentId, (item) => ({
        ...
        children: [
          ...item.children,
          createMenuItem(`child-${parentId}`, item.children.length + 1),
        ],
      })),
    );
  };
              ...
              {item.children.length > 0 && (
                <div
                  style={{
                    ...
                    borderInlineStart: "2px solid #334155",
                    display: "grid",
                    gap: 8,
                  }}
                >
                  {item.children.map((child) => (
                    ...
                    >
                      <input
                        value={child.title}
                        onChange={(event) =>
                          updateItem(child.id, { title: event.target.value })
                        }
                      ...
                      <input
                        value={child.url}
                        onChange={(event) =>
                          updateItem(child.id, { url: event.target.value })
                        }
