...
| **ProductCustomField** *(جديد)* | — | id, product_id, key, value, type (text/number/date/boolean) |
| **ProductPricingTier** *(جديد)* | — | id, product_id, min_quantity, price, customer_group_id |
| **ProductAttachment** *(جديد)* | — | id, product_id, name, file_url, file_type |
| **ProductSpec** *(جديد)* | — | id, product_id, key, value |
...
| **tags[]** *(مكتشف من الصور)* | ❌ ناقص |
| **assigned_employee_id** *(مكتشف من الصور)* | ❌ ناقص |
| **branch_id** *(مكتشف من الصور)* | ❌ ناقص |
...
| **CartItem.note** *(مكتشف من الصور)* | ❌ ناقص |
...
**حقول ThemeSettings الناقصة** *(مكتشفة من الصور)*:
...
**ThemeFeature** *(جديد — مكتشف من الصور)*:
...
**الحالة**: ❌ غائب تماماً *(مؤكد بصرياً — أيقونة ♡ على كل بطاقة منتج)*
...
**الحالة**: ❌ غائب *(مؤكد بصرياً — widget في Dashboard مع أسماء عملاء)*
...
## المجال 22: التقارير والتحليلات (Analytics & Reports) *(جديد)*
...
## المجال 23: الموظفون (Employees) *(جديد)*
...
## المجال 24: الحجوزات (Reservations) *(جديد)*
...
## المجال 25: رابط الدفع (Payment Link) *(جديد)*
...
## المجال 26: بوليصة الشحن (Shipping Label) *(جديد)*
...
## المجال 27: هدف الشهر (Monthly Goal) *(جديد)*
...
## المجال 28: الاشتري الآن وادفع لاحقاً (BNPL) *(جديد)*
...
## المجال 29: المؤثرون (Influencer) *(جديد)*
