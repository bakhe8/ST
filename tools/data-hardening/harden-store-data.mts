...
function sanitizeLegacyPlaceholdersDeep(
    value: any,
    ...
    if (Array.isArray(value)) {
        return value.map((entry) =>
            sanitizeLegacyPlaceholdersDeep(entry, summary),
        );
    }
...
function normalizeProductImages(
    ...
    const candidate = sanitizeImageUrl(
        raw?.main_image || raw?.image?.url || raw?.thumbnail,
        summary,
    );
    if (candidate) {
        return {
    ...
    return source.map((option: any, optionIndex: number) => ({
        ...
        values: Array.isArray(option?.values)
            ? option.values.map((value: any, valueIndex: number) => ({
                  id: String(
                      value?.id ||
                          `${productId}-opt-${optionIndex + 1}-val-${valueIndex + 1}`,
                  ),
...
function ensureProductCustomFields(raw: any) {
    const customFields = Array.isArray(raw?.custom_fields)
        ? raw.custom_fields.filter(Boolean)
        : [];
    if (customFields.length > 0) return customFields;
    return [{ key: "الضمان", value: "سنة" }];
}
...
async function main() {
    ...
    try {
        ...
        for (const store of stores) {
            ...
            for (const parsed of parsedCategories) {
                ...
                const parentRaw = String(
                    base?.parentId ?? base?.parent_id ?? "",
                ).trim();
                let normalizedParent: string | null =
                    parentRaw && parentRaw !== "root" ? parentRaw : null;

                if (
                    normalizedParent &&
                    (!categoryIdSet.has(normalizedParent) ||
                        normalizedParent === parsed.key)
                ) {
                ...
                const normalizedCategory = {
                    ...
                    order: Number.isFinite(Number(base?.order))
                        ? Number(base.order)
                        : 0,
                };
                ...
                const needsEntityKeyUpdate =
                    parsed.row.entityKey !== parsed.key;
                if (
                    parsed.row.payloadJson !== nextPayloadJson ||
                    needsEntityKeyUpdate
                ) {
            ...
            for (const row of brandRows) {
                ...
                const sanitizedBase = sanitizeLegacyPlaceholdersDeep(
                    base,
                    summary,
                );
                ...
                const normalizedBrand = {
                    ...
                    image: {
                        ...(sanitizedBase?.image &&
                        typeof sanitizedBase.image === "object"
                            ? sanitizedBase.image
                            : {}),
                        url: logoCandidate,
                    },
                ...
                if (
                    row.payloadJson !== nextPayloadJson ||
                    needsEntityKeyUpdate
                ) {
                    if (needsEntityKeyUpdate) {
                        summary.normalizedEntityKeys += 1;
            ...
            for (const row of productRows) {
                ...
                const validCategoryIds = categoryIds.filter((id) =>
                    categoryLookup.has(id),
                );
                summary.removedDanglingCategoryRefs += Math.max(
                    0,
                    categoryIds.length - validCategoryIds.length,
                );
                ...
                const imagePack = normalizeProductImages(
                    sanitizedBase,
                    key,
                    summary,
                );
                ...
                const normalizedPrice = normalizePriceValue(
                    sanitizedBase?.price,
                    currency,
                );
                const normalizedRegularPrice = normalizePriceValue(
                    sanitizedBase?.regular_price || normalizedPrice,
                    currency,
                );
                const normalizedSalePrice = normalizePriceValue(
                    sanitizedBase?.sale_price || normalizedPrice,
                    currency,
                );
        ...
        if (!args.write) {
            console.log(
                "[HARDEN] Dry run only. Re-run with --write to persist changes.",
            );
        }
    } finally {
