...
const run = async () => {
    ...
    const storeId = String(args.storeId || process.env.VTDR_STORE_ID || '').trim();
    const themeId = String(args.themeId || process.env.VTDR_THEME_ID || 'theme-raed-master').trim();
    const version = String(args.version || process.env.VTDR_THEME_VERSION || '1.0.0').trim();
    ...
    if (!storeId) {
        console.error('[preview-baseline] Missing --storeId (or VTDR_STORE_ID env).');
        process.exitCode = 1;
        return;
    }
    ...
    const worker = async () => {
        while (samples.length + failed < runs) {
            ...
            try {
                ...
                if (!response.ok) {
                    failed += 1;
                    console.error(`[preview-baseline] Request ${idx} failed with ${response.status}`);
                    continue;
                }
                await response.text();
                const totalMs = now() - startedAt;
                samples.push(totalMs);
                const headerValue = Number(response.headers.get('x-vtdr-render-total-ms') || '0');
                if (Number.isFinite(headerValue) && headerValue > 0) {
                    serverRenderMs.push(headerValue);
                }
            } catch (error) {
                failed += 1;
                console.error(`[preview-baseline] Request ${idx} failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    };
