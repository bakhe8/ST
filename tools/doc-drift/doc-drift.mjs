...
function extractMountedFactories(indexSource) {
    ...
    const directMountRegex = /(apiRouter|app)\.use\(\s*'([^']+)'\s*,[^;\n]*?create([A-Za-z]+)Routes\(/g;
    let match;
    ...
    const variableMountRegex = /(apiRouter|app)\.use\(\s*'([^']+)'\s*,\s*([A-Za-z0-9_]+)\s*\)/g;
    let variableMatch;
...
function buildSchemaSnapshot() {
    ...
    const modelMatches = [...schemaSource.matchAll(/^\s*model\s+([A-Za-z0-9_]+)\s+\{/gm)];
    const models = modelMatches.map((m) => m[1]).sort((a, b) => a.localeCompare(b));
...
if (mode === 'sync') {
    ...
    console.log(`[DOC-DRIFT] Synced snapshots: ${API_SNAPSHOT_REL}, ${SCHEMA_SNAPSHOT_REL}`);
    process.exit(0);
}
...
if (!apiExpected) {
    ...
    if (missing.length || extra.length) {
        ...
        if (extra.length) {
            console.error(`  New/changed routes not documented (${extra.length}):`);
...
if (!schemaExpected) {
    ...
    const modelsMismatch = !arraysEqual(schemaExpected.models, schemaCurrent.models);
    if (hashMismatch || modelsMismatch) {
        ...
        if (modelsMismatch) {
            console.error(`  Expected models: ${schemaExpected.models.join(', ')}`);
            console.error(`  Current  models: ${schemaCurrent.models.join(', ')}`);
        }
    }
}

if (failed) {
    console.error('[DOC-DRIFT] Run: npm run docs:sync then update documentation files in Docs/Specifications.');
    process.exit(1);
}
