...
export const BrandingSchema = z.object({
    colors: z.object({
        primary: z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/).describe('Primary color in hex format').optional(),
        secondary: z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/).describe('Secondary color in hex format').optional(),
        text: z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/).optional(),
        background: z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/).optional(),
    }).optional(),
    logo: z.union([z.string().url(), z.object({ url: z.string().url(), alt: z.string().optional() })]).optional(),
