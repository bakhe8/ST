  ...
  cartItems.forEach((item) => {
    ...
    if (productOptions) {
      productOptions.addEventListener("changed", (e) => {
        setTimeout(() => {
          if (!item.reportValidity() || e.detail?.event?.type == "added")
            return;
...
function observeQuantityChanges(quantityComponent, itemId, item) {
  const observer = new MutationObserver(() => {
    const quantityInput = quantityComponent.querySelector(
      'input[name="quantity"]',
    );
    if (quantityInput) {
...
function handleCartUpdateFailure(itemId, cartItems) {
  return salla.api.cart
    ...
    .then(({ data: { cart: cartDetails } }) => {
      const currentProduct = cartDetails.items.find(
        (item) => Number(item.id) === Number(itemId),
      );
      if (!currentProduct)
        throw new Error(`Product with ID ${itemId} not found in cart details.`);
...
function updateCartItemState(cartItems, currentProduct) {
  cartItems.forEach((item) => {
    ...
    if (Number(currentProduct.id) === Number(ID)) {
      ...
      if (productOptions)
        productOptions.setOptionsData(currentProduct.options, false);
  ...
  const targetItems = itemId
    ? [
        document.querySelector(
          `.main-content form:not(.cart-options form)#item-${itemId} .cart-item`,
        ),
      ]
