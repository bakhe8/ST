...
describe("SimulatorService theme component source normalization", () => {
  it("normalizes source aliases, infers missing sources, and backfills variable-list sources", async () => {
    const storeLogic = {
      ...
      getDataEntities: vi
        .fn()
        .mockImplementation(async (_storeId: string, type: string) => {
            ...
            blog_category: [
              {
                ...
                url: "/blog/categories/blog-category-1",
              },
            ],
            blog_categories: [],
            specialOffer: [
    ...
    const themeRuntimeAdapter = {
      ...
      resolvePlaceholderImage: vi
        .fn()
        .mockReturnValue("/images/placeholder.png"),
    } as any;
    ...
    const byId = new Map(
      fields.map((field: any) => [String(field?.id), field]),
    );
    ...
    const dynamicSources = (dynamicUrlField?.variableSources || []).map(
      (entry: any) => entry.value,
    );
    expect(dynamicSources).toEqual(
      expect.arrayContaining(["products", "blog_articles"]),
    );
    expect(Array.isArray(dynamicUrlField?.variableOptions?.products)).toBe(
      true,
    );
    expect(dynamicUrlField.variableOptions.products).toEqual(
      expect.arrayContaining([expect.objectContaining({ value: "prd-1" })]),
    );
    expect(Array.isArray(dynamicUrlField?.variableOptions?.blog_articles)).toBe(
      true,
    );
    expect(dynamicUrlField.variableOptions.blog_articles).toEqual(
      expect.arrayContaining([expect.objectContaining({ value: "post-1" })]),
    );

    const fallbackField: any = byId.get("fallback_url");
    const fallbackSources = (fallbackField?.variableSources || []).map(
      (entry: any) => entry.value,
    );
    expect(fallbackSources).toEqual(
      expect.arrayContaining(["products", "categories", "custom"]),
    );
