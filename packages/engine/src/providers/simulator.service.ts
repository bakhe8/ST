...
export class SimulatorService {
  ...
  private normalizeProductImages(data: any) {
    ...
    if (normalizedFromArray.length > 0) {
      const main =
        normalizedFromArray.find((img: any) => img.is_default) ||
        normalizedFromArray[0];
      return {
    ...
    const candidate = this.sanitizeImageUrl(
      data?.main_image || data?.image?.url || data?.thumbnail,
    );
    if (!candidate) {
      return {
        images: [],
  ...
  private normalizeProductOptions(data: any) {
    ...
    return options.map((option: any) => {
      const detailsSource = Array.isArray(option?.details)
        ? option.details
      ...
      const details = detailsSource.map((value: any, index: number) => {
        ...
        const name = String(
          value?.name || value?.label || `Value ${index + 1}`,
        );
        const additionalPrice = this.toNumber(
          value?.additional_price ?? value?.price,
          0,
        );
        return {
          ...value,
          ...
          full_name: String(
            value?.full_name ||
              (additionalPrice > 0 ? `${name} (+${additionalPrice})` : name),
          ),
      ...
      const visibilityConditionObject =
        option?.visibility_condition &&
        typeof option.visibility_condition === "object"
          ? option.visibility_condition
          : undefined;
      const visibilityConditionType = String(
        option?.visibility_condition_type ||
          option?.condition_type ||
          visibilityConditionObject?.type ||
          "",
      ).trim();
      const visibilityConditionOption = String(
        option?.visibility_condition_option ||
          option?.condition_option ||
          option?.condition_attributes ||
          visibilityConditionObject?.option ||
          "",
      ).trim();
      const visibilityConditionValue = String(
        option?.visibility_condition_value ||
          option?.condition_value ||
          visibilityConditionObject?.value ||
          "",
      ).trim();
      const visibilityCondition =
        ...
        visibilityConditionValue
          ? {
              ...
              typeof visibilityConditionObject === "object"
                ? visibilityConditionObject
                : {}),
              type:
                visibilityConditionType ||
      ...
      const advance =
        option?.advance &&
        typeof option.advance === "object" &&
        !Array.isArray(option.advance)
          ? option.advance
          : {};
      ...
      return {
        ...
        description: this.pickLocalizedText(option?.description || ""),
        sort: this.toNumber(option?.sort, 0),
        display_type: this.pickLocalizedText(
          option?.display_type ||
            option?.displayType ||
            option?.type ||
            "select",
        ...
        associated_with_order_time: this.toBoolean(
          option?.associated_with_order_time ?? option?.associatedWithOrderTime,
          false,
        ),
        not_same_day_order: this.toBoolean(option?.not_same_day_order, false),
        availability_range: this.toBoolean(option?.availability_range, false),
        from_date_time: this.pickLocalizedText(option?.from_date_time || ""),
        to_date_time: this.pickLocalizedText(option?.to_date_time || ""),
        visibility_condition_type: visibilityConditionType || undefined,
        visibility_condition_option: visibilityConditionOption || undefined,
        visibility_condition_value: visibilityConditionValue || undefined,
        visibility_condition: visibilityCondition,
        condition_attributes: visibilityConditionOption || undefined,
        advance: {
          placeholder: this.pickLocalizedText(advance?.placeholder || ""),
          inner_label: this.pickLocalizedText(advance?.inner_label || ""),
          min_length: this.toNumber(advance?.min_length, 0),
          max_length: this.toNumber(advance?.max_length, 0),
          multiple_file: this.toBoolean(advance?.multiple_file, false),
        },
  ...
  private normalizeProductVariants(data: any) {
    ...
    return variants.map((variant: any) => {
      ...
      const isAvailable =
        ...
        (isInfiniteQuantity || (quantity || 0) > 0);
      return {
        ...
        max_quantity: isInfiniteQuantity
          ...
          : Math.max(
              1,
              this.normalizeQuantity(variant?.max_quantity, quantity || 1) || 1,
            ),
        price,
  ...
  private normalizeProductNotifyAvailability(base: any, shouldEnable: boolean) {
    ...
    if (
      base?.notify_availability &&
      typeof base.notify_availability === "object"
    ) {
      const source = base.notify_availability;
      const channels =
        Array.isArray(source.channels) && source.channels.length > 0
          ? source.channels
              .map((entry: any) => this.pickLocalizedText(entry))
              .filter(Boolean)
          : ["sms", "email"];
      return {
        ...source,
        ...
        subscribed_options: Array.isArray(source.subscribed_options)
          ? source.subscribed_options
          : [],
  ...
  private resolveProductInventory(base: any, variants: any[]) {
    ...
    const variantHasInfinite =
      hasVariants &&
      variants.some((variant: any) => Boolean(variant?.is_infinite_quantity));
      ...
      isInfiniteQuantity || !trackQuantity
        ...
        : this.normalizeQuantity(
            base?.low_stock_threshold ??
              base?.low_stock_limit ??
              base?.minimum_quantity,
            0,
          );

    const explicitMax = this.normalizeQuantity(base?.max_quantity, 0);
    const soldQuantity = this.normalizeQuantity(
      base?.sold_quantity ?? base?.sales_count,
      0,
    );
    ...
    const explicitAvailability =
      typeof base?.is_available === "boolean" ? base.is_available : undefined;
    ...
    let isAvailable =
      explicitAvailability ?? (hasAvailableStock || allowBackorder);
    let isOutOfStock =
      !isAvailable ||
      (trackQuantity &&
        !isInfiniteQuantity &&
      ...
      isInfiniteQuantity || !trackQuantity
        ...
        : allowBackorder
          ...
          : Math.max(
              1,
              Math.min(
                explicitMax || availableQuantity || 0 || 1,
                availableQuantity || 0 || explicitMax || 1,
              ),
            );
    ...
    if (rawStatus === "hidden") {
      ...
      inventoryStatus =
        allowBackorder && isAvailable ? "backorder" : "out_of_stock";
    } else if (
      lowStockThreshold > 0 &&
      (availableQuantity || 0) <= lowStockThreshold
    ) {
    ...
    let status =
      inventoryStatus === "out_of_stock"
        ? rawStatus === "out-and-notify" || Boolean(base?.notify_availability)
          ? "out-and-notify"
          : "out"
        : "sale";
    if (rawStatus === "hidden") {
    ...
    return {
      ...
      status,
      add_to_cart_label: this.pickLocalizedText(
        base?.add_to_cart_label || defaultAddToCartLabel,
      ),
      has_options:
  ...
  private async normalizeProductCategories(
    storeId: string,
    categoriesInput: any,
  ) {
    ...
    const allCategories = await this.storeLogic.getDataEntities(
      storeId,
      "category",
    );
  ...
  private toReferenceTokens(value: any): string[] {
    ...
    try {
      ...
      // Raw token already processed above.
    }

    return Array.from(tokens);
  }

  private referencesIntersect(target: any, candidates: any[]): boolean {
    const targetTokens = new Set(this.toReferenceTokens(target));
    if (targetTokens.size === 0) return false;

    for (const candidate of candidates || []) {
      const candidateTokens = this.toReferenceTokens(candidate);
  ...
  private isGenericProductsSourceValue(value: any): boolean {
    const tokens = this.toReferenceTokens(value);
  ...
  private collectProductCategoryReferenceCandidates(product: any): string[] {
    const refs: any[] = [];
    refs.push(...this.collectProductCategoryIds(product));
    ...
    categories.forEach((category: any) => {
      if (category && typeof category === "object") {
        refs.push(category.id, category.slug, category.url);
      } else {
        refs.push(category);
      }
    });
    ...
    refs.push(product?.category_id);

    return Array.from(
      new Set(refs.filter((entry) => String(entry || "").trim())),
    );
  }

  private collectProductBrandReferenceCandidates(product: any): string[] {
    const brand =
      product?.brand && typeof product.brand === "object" ? product.brand : {};
    const refs = [product?.brand_id, brand.id, brand.slug, brand.url];
    return Array.from(
      new Set(refs.filter((entry) => String(entry || "").trim())),
    );
  }

  private collectBlogArticleCategoryReferenceCandidates(
    article: any,
  ): string[] {
    const category =
  ...
  private normalizeProductDonation(base: any, fallbackAmount: number) {
    const donationSource =
      base?.donation &&
      typeof base.donation === "object" &&
      !Array.isArray(base.donation)
        ? base.donation
        : {};
    ...
    const isDonation = this.toBoolean(
      base?.is_donation ??
        String(base?.type || "").toLowerCase() === "donating",
      hasDonationSignal,
    );
    ...
    const computedPercent =
      targetAmount > 0
        ? Math.min(
            100,
            this.roundAmount((collectedAmount / targetAmount) * 100),
          )
        : 0;
    ...
    const parsedEndDate = targetEndDate
  ...
  private async normalizeProductPayload(
    ...
    const categories = await this.normalizeProductCategories(
      storeId,
      categoriesFromInput,
    );
    const normalizedCategories = (categories.categories || []).map(
      (entry: any) =>
        this.normalizeCategoryPayload(
          entry,
          String(entry?.id || this.generateEntityId("category")),
        ),
    );
    const currency = String(
      base?.price?.currency ||
        base?.sale_price?.currency ||
        base?.regular_price?.currency ||
        "SAR",
    );
    const price = this.normalizePriceValue(base?.price, currency);
    const regularPrice = this.normalizePriceValue(
      base?.regular_price || { amount: price.amount },
      currency,
    );
    const salePrice = this.normalizePriceValue(
      base?.sale_price || { amount: price.amount },
      currency,
    );
    ...
    const url = this.normalizeEntityUrl(base?.url, `/products/${slug || key}`, [
      "/product",
      "/products",
    ]);
    const firstCategory = normalizedCategories[0] || null;
    const startingPriceAmount =
      normalizedVariants.length > 0
        ? normalizedVariants
            .map((entry: any) =>
              this.toNumber(entry?.price?.amount, salePrice.amount),
            )
            .filter((entry: number) => Number.isFinite(entry))
            .sort((a: number, b: number) => a - b)[0]
        : undefined;
    const baseCurrencyPrice = this.toNumber(
      base?.base_currency_price ?? salePrice.amount ?? price.amount,
      salePrice.amount,
    );
    const weight = Math.max(
      0,
      this.toNumber(base?.weight ?? base?.shipping_weight, 0),
    );
    const weightUnitCandidate = this.pickLocalizedText(
      (base?.weight_unit ?? base?.weightUnit) || "kg",
    ).toLowerCase();
    ...
    const rawMetadata =
      ...
      !Array.isArray(base.metadata)
        ...
        : {};
    const metadata = {
      title: this.pickLocalizedText(
        rawMetadata?.title || base?.seo_title || "",
      ),
      description: this.pickLocalizedText(
        rawMetadata?.description || base?.seo_description || "",
      ),
      keywords: Array.isArray(rawMetadata?.keywords)
        ? rawMetadata.keywords
            .map((entry: any) => this.pickLocalizedText(entry))
            .filter(Boolean)
        : this.pickLocalizedText(rawMetadata?.keywords || "")
            .split(",")
            .map((entry: string) => entry.trim())
            .filter(Boolean),
      image:
        this.sanitizeImageUrl(rawMetadata?.image || base?.seo_image || "") ||
        undefined,
    };
    ...
    const customFields = customFieldsSource
      .map((entry: any, index: number) => ({
        ...
        key: this.pickLocalizedText(
          entry?.key || entry?.name || `field_${index + 1}`,
        ),
    ...
    const specs = specsSource
      .map((entry: any, index: number) => ({
        ...
        key: this.pickLocalizedText(
          entry?.key || entry?.name || `spec_${index + 1}`,
        ),
        value: this.pickLocalizedText(entry?.value || ""),
      }))
      .filter((entry: any) => entry.key);
    const attachmentsSource = Array.isArray(base?.attachments)
      ? base.attachments
      : [];
    const attachments = attachmentsSource
      .map((entry: any) => {
        const fileUrl = this.sanitizeImageUrl(
          entry?.file_url || entry?.url || entry?.file || "",
        );
        return {
          id: String(entry?.id || this.generateEntityId("attachment")),
          name: this.pickLocalizedText(
            entry?.name || entry?.title || "Attachment",
          ),
          file_url: fileUrl,
          file_type: this.pickLocalizedText(
            entry?.file_type || entry?.type || "",
          ),
        };
      })
      .filter((entry: any) => entry.file_url);
    const donation = this.normalizeProductDonation(
      base,
    ...
    const minAmountDonating = donation
      ? Math.max(
          ...
          this.toNumber(
            ...
            Math.max(
              1,
              this.roundAmount((salePrice.amount || price.amount || 1) * 0.1),
            ),
          ),
        )
      : undefined;
    const maxAmountDonating = donation
      ? Math.max(
          this.toNumber(
            base?.max_amount_donating,
            donation.target_amount || minAmountDonating || 1,
          ),
          minAmountDonating || 1,
        )
      : undefined;
    ...
    return {
      ...
      starting_price:
        Number.isFinite(startingPriceAmount) && startingPriceAmount > 0
          ? startingPriceAmount
          : undefined,
      ...
      image:
        images.images?.[0] ||
        (images.main_image ? { url: images.main_image, alt: name } : undefined),
      options: normalizedOptions,
      variants: normalizedVariants,
      ...
      is_require_shipping:
        !isDonation &&
        base?.is_require_shipping !== false &&
        productType !== "digital",
    };
  }
  ...
  private normalizeCategoryPayload(data: any, key: string) {
    ...
    const parent_id =
      parentValue && parentValue !== "root" ? parentValue : null;
    const name = this.pickLocalizedText(base?.name || base?.title || key);
    ...
    const image =
      this.sanitizeImageUrl(base?.image || base?.icon || base?.cover?.url) ||
  ...
  private normalizeBrandPayload(data: any, key: string) {
    ...
    const logo =
      this.sanitizeImageUrl(base?.logo || base?.image?.url) ||
      this.defaultProductPlaceholder;
    const banner = this.sanitizeImageUrl(
      base?.banner || base?.cover?.url || base?.header_image,
    );
    const url = this.normalizeEntityUrl(base?.url, `/brands/${slug || key}`, [
      "/brand",
      "/brands",
    ]);
  ...
  private normalizeOfferCategories(base: any) {
    ...
    if (Array.isArray(base?.category_ids)) {
      sourceEntries.push(
        ...base.category_ids.map((entry: any) => ({ id: entry })),
      );
    }
    ...
    sourceEntries.forEach((entry: any, index: number) => {
      const payload =
        entry && typeof entry === "object" && !Array.isArray(entry)
          ...
          : { id: entry };
      const key =
        String(payload?.id ?? payload?.category_id ?? "").trim() ||
        this.generateEntityId("offer_category");
  ...
  private normalizeOfferProducts(base: any) {
    ...
    if (Array.isArray(base?.product_ids)) {
      sourceEntries.push(
        ...base.product_ids.map((entry: any) => ({ id: entry })),
      );
    }
    ...
    sourceEntries.forEach((entry: any, index: number) => {
      const payload =
        entry && typeof entry === "object" && !Array.isArray(entry)
          ...
          : { id: entry };
      const key =
        String(payload?.id ?? payload?.product_id ?? "").trim() ||
        this.generateEntityId("offer_product");
      const previous = productsById.get(key) || {};
      const name = this.pickLocalizedText(
        payload?.name ||
          payload?.title ||
          previous?.name ||
          `Product ${index + 1}`,
      ...
      const slug = this.slugify(
        payload?.slug || previous?.slug || name || key,
        "product",
      );
      const url = this.normalizeEntityUrl(
      ...
      const price = this.normalizePriceValue(
        payload?.price ?? payload?.sale_price ?? previous?.price,
        currency,
      );
      const regularPrice = this.normalizePriceValue(
        payload?.regular_price ??
          previous?.regular_price ?? { amount: price.amount },
        currency,
      );
      const salePrice = this.normalizePriceValue(
        payload?.sale_price ?? previous?.sale_price ?? { amount: price.amount },
        currency,
      );
      ...
      const categories = normalizedCategoryIds.map((id: string) => ({
        id,
        name: id,
      }));
      ...
      productsById.set(key, {
        ...
        url,
        description: this.pickLocalizedText(
          payload?.description || previous?.description || "",
        ),
        status: this.pickLocalizedText(
          payload?.status || previous?.status || "sale",
        ),
        type: this.pickLocalizedText(
          payload?.type || previous?.type || "product",
        ),
        price,
        regular_price: regularPrice,
        sale_price: salePrice,
  ...
  private normalizeOfferPayload(data: any, key: string) {
    ...
    const image =
      this.sanitizeImageUrl(base?.image || base?.cover?.url) ||
      this.defaultProductPlaceholder;
    const discountValue = this.toNumber(
      base?.discount_value ?? base?.value ?? base?.discount,
      0,
    );
    ...
    const url = this.normalizeEntityUrl(base?.url, `/offers/${slug || key}`, [
      "/offer",
      "/offers",
    ]);
    ...
    return {
      ...
      discount_type: this.pickLocalizedText(
        base?.discount_type || base?.type || "percentage",
      ),
      discount_value: discountValue,
      starts_at: this.pickLocalizedText(
        base?.starts_at || base?.start_at || "",
      ),
  ...
  private normalizeMenuNode(
    ...
    const title = this.pickLocalizedText(
      base?.title || base?.name || `رابط ${fallbackOrder}`,
    );
  ...
  private async buildDefaultMenu(storeId: string, type: string) {
    const normalizedType = String(type || "header")
      .trim()
      .toLowerCase();
    ...
    const topCategories = (categories || [])
      .filter(
        (category: any) =>
          !String(category?.parent_id || category?.parentId || "").trim(),
      )
      .slice(0, 6)
        ...
        this.normalizeMenuNode(
          {
            ...
            title: this.pickLocalizedText(
              category?.name || category?.title || `تصنيف ${index + 1}`,
            ),
            url: this.pickLocalizedText(
              category?.url || `/categories/${category?.id || index + 1}`,
            ),
            order: index + 1,
          },
          `category-${index + 1}`,
        ...
        this.normalizeMenuNode(
          {
            ...
            title: this.pickLocalizedText(
              page?.title || page?.name || `صفحة ${index + 1}`,
            ),
            url: this.pickLocalizedText(
              page?.url || `/${page?.slug || page?.id || `page-${index + 1}`}`,
            ),
            order: index + 1,
          },
          `page-${index + 1}`,
  ...
  private resolveProductAvailableQuantity(product: any) {
    ...
    if (
      ...
      Number.isFinite(product.available_quantity)
    ) {
    ...
    const reserved = this.toNumber(
      product?.reserved_quantity ?? product?.reserved,
      0,
    );
  ...
  private normalizeEntityUrl(
    ...
    const normalizedPathOnly =
      path.split("?")[0].split("#")[0].replace(/\/+$/g, "") || "/";
    ...
    if (
      normalizedPathOnly === "/" ||
      genericSet.has(normalizedPathOnly.toLowerCase())
    ) {
      return fallback;
    }
  ...
  private pickQueryList(
    ...
    const appendValue = (raw: any) => {
      if (raw == null) return;
      if (Array.isArray(raw)) {
        raw.forEach((entry) => appendValue(entry));
        return;
      }
  ...
  private getProductFromReferenceMap(
    productsByRef: Map<string, any>,
    ...candidateRefs: any[]
  ) {
  ...
  private resolveProductCartLimit(product: any) {
    ...
    const isInfiniteQuantity =
      Boolean(product?.is_infinite_quantity) || !trackQuantity;
  ...
  private sortProducts(items: any[], rawSort: string) {
    const sort = String(rawSort || "ourSuggest")
      .trim()
      .toLowerCase();
    const sorted = [...items];

    if (
      sort === "pricefromtoptolow" ||
      sort === "price_desc" ||
      sort === "price-desc" ||
      ...
      return sorted.sort(
        (a, b) =>
          this.resolveProductPriceAmount(b) - this.resolveProductPriceAmount(a),
      );
    }
    ...
    if (
      ...
      return sorted.sort(
        (a, b) =>
          this.resolveProductPriceAmount(a) - this.resolveProductPriceAmount(b),
      );
    }
    ...
    if (
      ...
      return sorted.sort(
        (a, b) =>
          this.toNumber(b?.sold_quantity ?? b?.sales_count, 0) -
          this.toNumber(a?.sold_quantity ?? a?.sales_count, 0),
      );
    }
    ...
    if (
      sort === "stockfromtoptolow" ||
      sort === "stock_desc" ||
      sort === "stock-desc" ||
      sort === "stock_high_to_low"
    ) {
      return sorted.sort(
        (a, b) =>
          this.resolveProductAvailableQuantity(b) -
          this.resolveProductAvailableQuantity(a),
      );
    }

    if (
      sort === "stockfromlowtotop" ||
      sort === "stock_asc" ||
      sort === "stock-asc" ||
      sort === "stock_low_to_high"
    ) {
      return sorted.sort(
        (a, b) =>
          this.resolveProductAvailableQuantity(a) -
  ...
  private normalizeReviewPayload(data: any, key: string) {
    ...
    const productId = String(
      base?.product_id ?? base?.productId ?? base?.product?.id ?? "",
    ).trim();
    const blogId = String(
      base?.blog_id ?? base?.blogId ?? base?.blog?.id ?? "",
    ).trim();
    const type =
      String(base?.type || (blogId ? "blog" : "product"))
        .trim()
        .toLowerCase() || "product";
    const fallbackRating = type === "ask" ? 0 : 5;
    const rating = Math.min(
      ...
      Math.max(
        ...
        Math.round(
          this.toNumber(
            base?.rating ?? base?.stars ?? fallbackRating,
            fallbackRating,
          ),
        ),
      ),
    ...
    const stars = rating;
    const orderId = String(base?.order_id ?? base?.orderId ?? "").trim();
    const customerName = this.pickLocalizedText(
      base?.customer_name || base?.customer?.name || "عميل المتجر",
    );
    const customerAvatar =
      this.sanitizeImageUrl(
        base?.customer_avatar || base?.customer?.avatar || "",
      ) || this.defaultProductPlaceholder;
    const customerEmail = this.pickLocalizedText(
      base?.customer_email || base?.customer?.email || "",
    );
    const content = this.pickLocalizedText(
      base?.content || base?.comment || base?.text || "",
    );
    const reply = this.pickLocalizedText(base?.reply || base?.answer || "");
    const isPublished =
      base?.is_published != null
        ? Boolean(base.is_published)
        : this.toBoolean(base?.publish ?? base?.published, true);
    ...
    return {
      ...
      created_at: this.pickLocalizedText(
  ...
  private normalizeQuestionPayload(data: any, key: string) {
    ...
    const customerName = this.pickLocalizedText(
      base?.customer_name || base?.customer?.name || "زائر",
    );
    const customerAvatar =
      this.sanitizeImageUrl(
        base?.customer_avatar || base?.customer?.avatar || "",
      ) || this.defaultProductPlaceholder;
    const question = this.pickLocalizedText(
      base?.question || base?.title || base?.content || base?.text || "",
    );
    const answer = this.pickLocalizedText(base?.answer || base?.reply || "");
    const isAnswered =
      base?.is_answered != null ? Boolean(base.is_answered) : Boolean(answer);
    ...
    return {
      ...
      answered_at: isAnswered
        ? this.pickLocalizedText(
            base?.answered_at || base?.updated_at || new Date().toISOString(),
          )
        : "",
    };
  }
  ...
  private async resolveFeedbackProductId(storeId: string, rawProductId: any) {
    ...
    if (directId) {
      const existing = await this.storeLogic.getDataEntity(
        storeId,
        "product",
        directId,
      );
  ...
  private async refreshProductFeedbackMetrics(
    storeId: string,
    productId: string,
  ) {
    ...
    const currentProduct = await this.storeLogic.getDataEntity(
      storeId,
      "product",
      normalizedProductId,
    );
    ...
    const reviews = (reviewsRaw || [])
      ...
      .filter(
        (entry: any) =>
          entry.product_id === normalizedProductId &&
    ...
    const comments = reviews.map((review: any) => ({
      ...
      customer: {
        name: this.pickLocalizedText(
          review.customer_name || review.customer?.name || "عميل المتجر",
        ),
        avatar: this.pickLocalizedText(
    ...
    const ratingStars =
      ratingCount > 0
        ? Number(
            (
              comments.reduce(
                (sum: number, review: any) => sum + Number(review.stars || 0),
                0,
              ) / ratingCount
            ).toFixed(2),
          )
    ...
    const questionsView = questions.map((question: any) => ({
      ...
      customer: {
        name: this.pickLocalizedText(
          question.customer_name || question.customer?.name || "زائر",
        ),
        avatar: this.pickLocalizedText(
          question.customer_avatar ||
    ...
    await this.storeLogic.upsertDataEntity(
      storeId,
      "product",
      normalizedProductId,
      payload,
  ...
  private async normalizeBlogArticlePayload(
    storeId: string,
    data: any,
    key: string,
  ) {
    ...
    const categoryId = String(
      base?.category_id ?? base?.categoryId ?? base?.category?.id ?? "",
    ).trim();
    const categories = await this.getBlogCategoriesRaw(storeId);
    const matchedCategory = categories.find(
      (entry: any) => String(entry?.id || "") === categoryId,
    );
    const url = this.normalizeEntityUrl(base?.url, `/blog/${slug || key}`, [
      "/blog",
      "/article",
      "/articles",
    ]);
    ...
    return {
      ...
      category: matchedCategory
        ? {
            ...
            slug: this.pickLocalizedText(matchedCategory?.slug || ""),
            url: this.pickLocalizedText(matchedCategory?.url || ""),
          }
        : base?.category && typeof base.category === "object"
          ? {
              id: String(base.category.id || categoryId || ""),
              name: this.pickLocalizedText(
                base.category.name || base.category.title || "",
              ),
              slug: this.pickLocalizedText(base.category.slug || ""),
              url: this.pickLocalizedText(base.category.url || ""),
            }
          : undefined,
      published_at: this.pickLocalizedText(
        base?.published_at || base?.created_at || new Date().toISOString(),
      ),
      is_published: base?.is_published !== false,
    };
  }
  ...
  private inferFieldSource(field: any) {
    ...
    if (/blog.*categor|post.*categor|تصنيف.*مدون/.test(hints))
  ...
  private buildDefaultVariableSources(
    ...
    return Array.from(
      new Set(
        base.map((entry) => this.normalizeSourceKey(entry)).filter(Boolean),
      ),
    )
  ...
  private getSourceEntityKey(item: any, fallbackPrefix: string) {
    const candidate =
      item?.id ?? item?.slug ?? item?.key ?? item?.code ?? item?.name;
  ...
  private getSourceEntityUrl(sourceKey: string, item: any) {
    ...
    if (source === "products")
      return this.normalizeEntityUrl(
        item?.url,
        `/products/${idOrSlug || "item"}`,
        ["/product", "/products"],
      );
    if (source === "categories")
      return this.normalizeEntityUrl(
        item?.url,
        `/categories/${idOrSlug || "item"}`,
        ["/category", "/categories", "/cat", "/cats"],
      );
    if (source === "brands")
      return this.normalizeEntityUrl(
        item?.url,
        `/brands/${idOrSlug || "item"}`,
        ["/brand", "/brands"],
      );
    if (source === "offers")
      return this.normalizeEntityUrl(
        item?.url,
        `/offers/${idOrSlug || "item"}`,
        ["/offer", "/offers"],
      );
    if (source === "pages")
      return this.normalizeEntityUrl(
    ...
    if (source === "blog_articles")
      return this.normalizeEntityUrl(item?.url, `/blog/${idOrSlug || "item"}`, [
        "/blog",
        "/article",
        "/articles",
      ]);
    if (source === "blog_categories")
      return this.normalizeEntityUrl(
        item?.url,
        `/blog/categories/${idOrSlug || "item"}`,
        ["/blog/category", "/blog/categories", "/category", "/categories"],
      );
    if (source === "reviews")
      return this.pickLocalizedText(
        item?.url || `/products/${item?.product_id || ""}#reviews`,
      );
    if (source === "questions")
      return this.pickLocalizedText(
        item?.url || `/products/${item?.product_id || ""}#questions`,
      );
    if (source === "offers_link") return "/offers";
    if (source === "brands_link") return "/brands";
    if (source === "blog_link") return "/blog";
    return (
      this.toNavigablePath(item?.url) || this.pickLocalizedText(item?.url || "")
    );
  }
  ...
  private toVariableListOptions(sourceKey: string, items: any[]) {
    ...
    if (source === "brands_link") {
      return [
        { value: "brands_link", label: "الماركات التجارية", url: "/brands" },
      ];
    }
    if (source === "blog_link") {
    ...
    return localItems
      .map((item: any) => {
        ...
        return {
          value: id,
          label: this.pickLocalizedText(
            item?.name || item?.title || item?.slug || id,
          ),
          url: String(url || ""),
        };
      })
  ...
  private normalizeVariableListField(
    field: any,
    sources: Record<string, any[]>,
  ) {
      ...
      ? normalized.sources
          .map((entry: any) => {
            const value = this.normalizeSourceKey(
              entry?.value ?? entry?.key ?? entry?.id ?? entry?.source,
            );
            ...
            return {
              ...
              value,
              label: this.pickLocalizedText(
                entry?.label || this.getSourceDisplayLabel(value),
              ),
            };
          })
          .filter(Boolean)
    ...
    (effectiveEntries as any[]).forEach((entry: any) => {
      ...
      optionsBySource[value] = this.toVariableListOptions(
        value,
        sources[value] || [],
      );
    });
  ...
  private normalizeThemeComponentField(
    field: any,
    sources: Record<string, any[]>,
  ) {
    const normalized = { ...(field || {}) };
    ...
    const isSourceItemsField =
      normalized?.type === "items" &&
      String(normalized?.format || "") === "dropdown-list";
    ...
    const sourceKey = this.normalizeSourceKey(
      normalized?.source || inferredSource,
    );
  ...
  private async buildCartState(
    storeId: string,
    rawItems: any[],
    couponCode = "",
  ) {
    const { normalizedProducts, productsByRef } =
      await this.getNormalizedProductsByReference(storeId);

    const defaultCurrency = String(
      normalizedProducts?.[0]?.price?.currency ||
        normalizedProducts?.[0]?.currency ||
        "SAR",
    );
      ...
      .map((entry: any, index: number) => {
        ...
        const productId = String(
          product?.id ||
            rawProductRef ||
            base?.id ||
            this.generateEntityId("product"),
        ).trim();
        const itemId = String(
          base?.id || productId || this.generateEntityId("cart_item"),
        ).trim();
        ...
        const totalSpecialPrice = this.roundAmount(
          this.toNumber(base?.total_special_price, total) || total,
        );
        ...
        const fallbackSlug = this.slugify(
          product?.slug || productName || productId,
          "product",
        );
        const url = this.normalizeEntityUrl(
        ...
        const detailedOffers = Array.isArray(base?.detailed_offers)
          ? base.detailed_offers
          : [];
        const offer =
          base?.offer && typeof base.offer === "object"
            ? {
        ...
        return {
          ...
          type: this.pickLocalizedText(
            base?.type || product?.type || "product",
          ),
          is_available: product
            ? itemLimit.isAvailable
    ...
    const itemsCount = normalizedItems.reduce(
      (acc: number, item: any) =>
        acc + this.normalizeQuantity(item.quantity, 0),
      0,
    );
    const subTotal = this.roundAmount(
      normalizedItems.reduce(
        (acc: number, item: any) => acc + this.toNumber(item.total, 0),
        0,
      ),
    );
    const optionsTotal = 0;
    const normalizedCoupon = this.normalizeCartCouponCode(couponCode);
    const couponDiscount = normalizedCoupon
      ? this.roundAmount(subTotal * 0.1)
      : 0;
    const discount = couponDiscount;
    const taxBase = Math.max(
      0,
      this.roundAmount(subTotal + optionsTotal - discount),
    );
    ...
    const freeShippingRemaining = Math.max(
      0,
      this.roundAmount(freeShippingMinimum - taxBase),
    );
    ...
    return {
      ...
      is_require_shipping: normalizedItems.some(
        (item: any) => String(item?.type || "product") !== "digital",
      ),
      total,
      gift: {
        enabled: false,
  ...
  private async getOrCreateCart(storeId: string) {
    const current = await this.storeLogic.getDataEntity(
      storeId,
      "cart",
      "default",
    );
    if (current && typeof current === "object") {
      const rawItems = Array.isArray((current as any).items)
  ...
  public async addWishlistItem(storeId: string, data: any) {
    ...
    const stored = await this.persistWishlistState(storeId, {
      product_ids: nextIds,
    });
  ...
  public async addCartItem(storeId: string, data: any) {
    ...
    if (existingIndex >= 0) {
      ...
      const nextQuantity =
        this.normalizeQuantity(existing?.quantity, 0) + quantityToAdd;
      if (
        product &&
        !productLimit.isInfiniteQuantity &&
        productLimit.maxQuantity > 0 &&
        nextQuantity > productLimit.maxQuantity
      ) {
        return {
          status: 400,
          success: false,
      ...
      updatedItems[existingIndex] = {
        ...
        product_id: String(
          existing?.product_id || payload?.product_id || itemId,
        ),
      ...
      if (
        ...
        !productLimit.isInfiniteQuantity &&
        productLimit.maxQuantity > 0 &&
        quantityToAdd > productLimit.maxQuantity
      ) {
        return {
          status: 400,
          success: false,
    ...
    const normalized = await this.buildCartState(
      storeId,
      updatedItems,
      this.normalizeCartCouponCode(cart.coupon),
    );
  ...
  private buildCheckoutSummary(cart: any, shippingCostInput: any) {
    ...
    const shippingCost = this.toNumber(
      shippingCostInput,
      this.toNumber(cart?.real_shipping_cost, 0),
    );
    const total = this.roundAmount(
      Math.max(
        0,
        subtotal + optionsTotal - discount + taxAmount + shippingCost,
      ),
    );
  ...
  private normalizeCheckoutSession(cart: any, raw: any) {
    ...
    const shippingMethods =
      Array.isArray(base.available_shipping_methods) &&
      base.available_shipping_methods.length > 0
        ? base.available_shipping_methods
        : this.getDefaultCheckoutShippingMethods(currency);
    const paymentMethods =
      Array.isArray(base.available_payment_methods) &&
      base.available_payment_methods.length > 0
    ...
    const selectedShipping =
      shippingMethods.find(
        (method: any) => String(method?.id || "").trim() === selectedShippingId,
      ) || null;
    const shipping = {
      method_id: selectedShipping ? String(selectedShipping.id) : "",
      method_name: selectedShipping
        ? this.pickLocalizedText(selectedShipping.name)
        : "",
      cost: this.toNumber(
        base?.shipping?.cost,
        selectedShipping
          ? this.toNumber(selectedShipping.cost, 0)
          : this.toNumber(cart?.real_shipping_cost, 0),
      ),
      currency,
    };
    ...
    const selectedPayment =
      paymentMethods.find(
        (method: any) => String(method?.id || "").trim() === selectedPaymentId,
      ) || null;
    const payment = {
      method_id: selectedPayment ? String(selectedPayment.id) : "",
      method_name: selectedPayment
        ? this.pickLocalizedText(selectedPayment.name)
        : "",
    ...
    const draft = {
      id: checkoutId,
      status:
        String(base.status || "active") === "completed"
          ? "completed"
          : "active",
    ...
    const steps = ["address", "shipping", "payment", "review"].map(
      (id, index) => ({
        id,
        order: index + 1,
        is_current: id === step,
  ...
  public async startCheckoutSession(storeId: string, data: any = {}) {
    ...
    const patched = {
      ...
      customer: {
        ...
        email: this.pickLocalizedText(
          payload?.customer?.email ||
            payload?.email ||
            current?.customer?.email ||
            "",
        ),
        mobile: this.pickLocalizedText(
      ...
      address: {
        ...(current.address || {}),
        country: this.pickLocalizedText(
          payload?.address?.country ||
            payload?.country ||
            current?.address?.country ||
            "SA",
        ),
        city: this.pickLocalizedText(
          payload?.address?.city ||
            payload?.city ||
            current?.address?.city ||
            "",
        ),
        district: this.pickLocalizedText(
          payload?.address?.district ||
            payload?.district ||
            current?.address?.district ||
            "",
        ),
        street: this.pickLocalizedText(
          payload?.address?.street ||
            payload?.street ||
            current?.address?.street ||
            "",
        ),
        postal_code: this.pickLocalizedText(
  ...
  public async updateCheckoutShipping(storeId: string, data: any) {
    ...
    const selectedMethod = (current.available_shipping_methods || []).find(
      (entry: any) => String(entry?.id || "").trim() === requestedMethodId,
    );
  ...
  public async updateCheckoutPayment(storeId: string, data: any) {
    ...
    const selectedMethod = (current.available_payment_methods || []).find(
      (entry: any) => String(entry?.id || "").trim() === requestedMethodId,
    );
  ...
  private buildOrderFromCart(cart: any, overrides: any = {}) {
    ...
    const customer =
      overrides?.customer && typeof overrides.customer === "object"
        ? overrides.customer
        : {};
    const shipping =
      overrides?.shipping && typeof overrides.shipping === "object"
        ? overrides.shipping
        : {};
    const payment =
      overrides?.payment && typeof overrides.payment === "object"
        ? overrides.payment
        : {};
    ...
    return {
      ...
      items: (cart.items || []).map((item: any) => ({
        ...
        product_name: this.pickLocalizedText(
        ...
        product_image: this.pickLocalizedText(
          item?.product_image || this.defaultProductPlaceholder,
        ),
      })),
  ...
  public async sendOrderInvoice(storeId: string, data: any) {
    ...
    const order = requestedOrderId
      ? (orders || []).find(
          (entry: any) => String(entry?.id || "") === requestedOrderId,
        )
      : (orders || []).sort(
          (a: any, b: any) =>
    ...
    const email = this.pickLocalizedText(
  ...
  private resolveOrderStatusMeta(rawStatus: any) {
    const statusKey = String(rawStatus || "new")
      .trim()
      .toLowerCase();
  ...
  private normalizeOrderApiPayload(order: any) {
    ...
    const shipping =
      base?.shipping && typeof base.shipping === "object" ? base.shipping : {};
    const payment =
    ...
    const items = (Array.isArray(base?.items) ? base.items : []).map(
      (entry: any, index: number) => {
        const source = entry && typeof entry === "object" ? entry : {};
        const productId = String(
          source?.product_id || source?.id || this.generateEntityId("product"),
        );
        const quantity = this.normalizeQuantity(source?.quantity, 1) || 1;
        const unitPrice = this.toNumber(
          source?.price?.amount ?? source?.price,
          0,
        );
        const total = this.toNumber(
          source?.total?.amount ?? source?.total,
          this.roundAmount(unitPrice * quantity),
        );
        ...
        return {
          ...
          product_id: productId,
          name: this.pickLocalizedText(
            source?.product_name || source?.name || `منتج ${index + 1}`,
          ),
          image:
            this.sanitizeImageUrl(
              source?.product_image || source?.image?.url || source?.image,
            ) || this.defaultProductPlaceholder,
          ...
          product: {
            ...
            url: productUrl,
            type: this.pickLocalizedText(
              source?.product?.type || source?.type || "product",
            ),
          },
          product_reservations: Array.isArray(source?.product_reservations)
            ? source.product_reservations
            : [],
          detailed_offers: Array.isArray(source?.detailed_offers)
            ? source.detailed_offers
            : [],
        };
      },
    );
    ...
    const total = this.toNumber(
      base?.total,
      this.roundAmount(subtotal - discount + shippingCost + taxAmount),
    );
    ...
    const packageItems = items.map((item: any) => ({
      ...
      product: item.product || {
        id: item.product_id,
        url: this.normalizeEntityUrl("", `/products/${item.product_id}`, [
          "/product",
          "/products",
        ]),
        type: "product",
    ...
    return {
      ...
      url: this.normalizeEntityUrl(base?.url, `/customer/orders/${orderId}`, [
        "/orders",
        "/order",
      ]),
      created_at: String(base?.created_at || new Date().toISOString()),
      updated_at: String(
        base?.updated_at || base?.created_at || new Date().toISOString(),
      ),
      status,
      ...
      shipping: {
        id: this.pickLocalizedText(
          shipping?.method_id || shipping?.id || "shipping",
        ),
        name: this.pickLocalizedText(
          shipping?.method_name || shipping?.name || "شحن قياسي",
        ),
      ...
      packages: [
        {
          shipping_company: {
            name: this.pickLocalizedText(
              shipping?.method_name || shipping?.name || "شركة الشحن",
            ),
            logo: this.sanitizeImageUrl(shipping?.logo) || undefined,
            number: this.pickLocalizedText(shipping?.number || ""),
      ...
      is_pending_payment:
        String(base?.payment_status || "").toLowerCase() === "pending",
      pending_payment_ends_in:
        this.normalizeQuantity(base?.pending_payment_ends_in, 0) || 0,
      is_rated: Boolean(base?.is_rated),
      rating:
        base?.rating && typeof base.rating === "object" ? base.rating : null,
  ...
  public async getOrders(storeId: string, query: Record<string, any> = {}) {
    const page = this.pickQueryInt(
      query,
      ["page", "current_page", "currentPage"],
      1,
    );
    const perPage = this.pickQueryInt(
      query,
    ...
    return this.buildPaginatedResponse(
      this.mapToSchema(pageItems, "order"),
      total,
      page,
      perPage,
    );
  ...
  public async getNotifications(
    ...
    const notifications = await this.storeLogic.getDataEntities(
      storeId,
      "notification",
    );
    ...
    let normalized = (notifications || []).map((entry: any, index: number) => ({
      ...
      sub_title: this.pickLocalizedText(
        entry?.sub_title || entry?.subtitle || "",
      ),
    ...
    const pageItems = normalized.slice(start, start + perPage);
    return this.buildPaginatedResponse(
      this.mapToSchema(pageItems, "notification"),
      total,
      page,
      perPage,
    );
  ...
  public async getProducts(storeId: string, query: Record<string, any> = {}) {
    ...
    const source = this.pickQueryValue(query, [
      "source",
      "source_type",
      "sourceType",
      "page_slug",
    ]).toLowerCase();
    const sourceValue = this.pickQueryValue(query, [
      "source_value",
      "sourceValue",
      "value",
    ]);
    const categoryParam = this.pickQueryValue(query, [
      "category_id",
      "categoryId",
      "category",
    ]);
    const brandParam = this.pickQueryValue(query, [
      "brand_id",
      "brandId",
      "brand",
    ]);
    const search = this.pickQueryValue(query, [
      "search",
      "q",
      "keyword",
    ]).toLowerCase();
    const sort = this.pickQueryValue(
      query,
      ["sort", "sort_by", "sortBy", "order_by", "orderBy"],
      "ourSuggest",
    );
    const status = this.pickQueryValue(query, [
      "status",
      "availability",
      "stock",
    ]).toLowerCase();
    const stockState = this.pickQueryValue(query, [
      "stock_state",
      "inventory_status",
    ]).toLowerCase();
    const effectiveStatus = String(stockState || status).toLowerCase();
    const featured = this.pickQueryValue(query, [
      "featured",
      "is_featured",
      "featured_only",
    ]).toLowerCase();
    const minPrice = this.pickQueryNumber(query, [
      "min_price",
      "minPrice",
      "price_from",
      "priceFrom",
    ]);
    const maxPrice = this.pickQueryNumber(query, [
      "max_price",
      "maxPrice",
      "price_to",
      "priceTo",
    ]);
    const onSale = this.pickQueryValue(query, [
      "on_sale",
      "is_on_sale",
      "sale",
    ]).toLowerCase();
    ...
    const isBrandScopedSource = source.includes("brand");
    const isCategoryScopedSource = source.includes("category");
    const shouldInferCategoryFromSourceValue =
      isProductIndexSource &&
      !isBrandScopedSource &&
      !isCategoryScopedSource &&
      normalizedSourceValue !== "" &&
      !this.isGenericProductsSourceValue(sourceValue);
    const inferredCategoryId = shouldInferCategoryFromSourceValue
      ? sourceValue
      : "";
    const effectiveCategoryId = isCategoryScopedSource
      ? sourceValue || categoryParam
      : categoryParam || inferredCategoryId;
    const effectiveBrandId = isBrandScopedSource
      ? sourceValue || brandParam
      : brandParam;
    ...
    if (
      ...
      filtered = filtered.filter((product: any) =>
        wishlistIds.has(String(product?.id || "").trim()),
      );
    }
    ...
    if (
      ...
      if (!relatedRef) {
        ...
        const targetProduct = filtered.find((product: any) =>
          this.collectProductReferenceCandidates(product).some(
            (candidate) => candidate.toLowerCase() === normalizedRelatedRef,
          ),
        );
        ...
        if (!targetProduct) {
          ...
          const targetCategoryIds = new Set(
            this.collectProductCategoryIds(targetProduct),
          );
          const targetBrandId = String(
            targetProduct?.brand?.id || targetProduct?.brand_id || "",
          ).trim();

          const scoredProducts = filtered
            .filter(
              (product: any) => String(product?.id || "").trim() !== targetId,
            )
            .map((product: any) => {
              const categories = this.collectProductCategoryIds(product);
              const sharedCategoryScore = categories.reduce(
                (acc: number, categoryId: string) =>
                  acc + (targetCategoryIds.has(categoryId) ? 1 : 0),
                0,
              );
              const brandId = String(
                product?.brand?.id || product?.brand_id || "",
              ).trim();
              const sameBrandScore =
                targetBrandId && brandId === targetBrandId ? 2 : 0;
              return {
                product,
          ...
          const relatedProducts = scoredProducts.filter(
            (entry: any) => entry.score > 0,
          );
          const scopedProducts =
            relatedProducts.length > 0 ? relatedProducts : scoredProducts;
          filtered = scopedProducts
            .sort((a: any, b: any) => {
              if (b.score !== a.score) return b.score - a.score;
              const featuredDelta =
                Number(Boolean(b.product?.is_featured)) -
                Number(Boolean(a.product?.is_featured));
              if (featuredDelta !== 0) return featuredDelta;
              return (
                this.resolveProductPriceAmount(b.product) -
                this.resolveProductPriceAmount(a.product)
              );
            })
    ...
    if (effectiveCategoryId) {
      ...
      filtered = filtered.filter((product: any) => {
        const refs = this.collectProductCategoryReferenceCandidates(product);
        return this.referencesIntersect(target, refs);
      });
    }
    ...
    if (effectiveBrandId) {
      ...
      filtered = filtered.filter((product: any) => {
        const refs = this.collectProductBrandReferenceCandidates(product);
        return this.referencesIntersect(target, refs);
      });
    }
    ...
    if (effectiveStatus) {
      if (
        ...
        filtered = filtered.filter((product: any) => {
          ...
          if (rawStatus === "hidden" || inventoryStatus === "hidden")
            return false;
          if (this.isProductBackorderState(product)) return false;
          if (inventoryStatus)
            return (
        ...
        filtered = filtered.filter((product: any) =>
          this.isProductLowStockState(product),
        );
      } else if (
        effectiveStatus === "backorder" ||
        effectiveStatus === "preorder" ||
        effectiveStatus === "pre_order"
      ) {
        filtered = filtered.filter((product: any) =>
          this.isProductBackorderState(product),
        );
      }
    }
    ...
    if (typeof minPrice === "number") {
      filtered = filtered.filter(
        (product: any) => this.resolveProductPriceAmount(product) >= minPrice,
      );
    }
    if (typeof maxPrice === "number") {
      filtered = filtered.filter(
        (product: any) => this.resolveProductPriceAmount(product) <= maxPrice,
      );
    }
  ...
  public async getProduct(storeId: string, productId: string) {
    ...
    const normalized = await this.normalizeProductPayload(
      storeId,
      product,
      String(product.id),
    );
  ...
  public async getCategories(storeId: string) {
    const categories = await this.storeLogic.getDataEntities(
      storeId,
      "category",
    );
  ...
  public async getCategory(storeId: string, categoryId: string) {
    ...
    const normalized = this.normalizeCategoryPayload(
      category,
      String(category.id),
    );
    return this.wrapResponse(this.mapToSchema(normalized, "category"));
  ...
  public async deleteCategory(storeId: string, categoryId: string) {
    ...
    for (const product of products || []) {
      ...
      if (
        filteredCategoryIds.length === (normalized.category_ids || []).length
      ) {
        continue;
      }
  ...
  public async updateBrand(storeId: string, brandId: string, data: any) {
    ...
    const payload = this.normalizeBrandPayload(
      { ...(current || {}), ...(data || {}) },
      brandId,
    );
  ...
  public async getOffer(storeId: string, offerId: string) {
    ...
    const offer = (offers || []).find(
      (entry: any) => String(entry?.id || "") === String(offerId),
    );
  ...
  public async updateOffer(storeId: string, offerId: string, data: any) {
    const currentPrimary = await this.storeLogic.getDataEntity(
      storeId,
      "specialOffer",
      offerId,
    );
    const currentFallback =
      currentPrimary ||
      (await this.storeLogic.getDataEntity(storeId, "offer", offerId));
    const payload = this.normalizeOfferPayload(
  ...
  public async getReviews(storeId: string, query: Record<string, any> = {}) {
    ...
    const keyword = this.pickQueryValue(query, [
      "keyword",
      "search",
      "q",
    ]).toLowerCase();
    const typeFilters = this.pickQueryList(query, ["type", "types"]).map(
      (entry) => entry.toLowerCase(),
    );
    const productFilters = this.pickQueryList(query, [
      "products",
      "product_id",
      "productId",
      "product",
    ]);
    const blogFilters = this.pickQueryList(query, [
      "blogs",
      "blog_id",
      "blogId",
      "blog",
    ]);
    const customerFilters = this.pickQueryList(query, [
      "customers",
      "customer_id",
      "customerId",
      "customer",
    ]).map((entry) => entry.toLowerCase());
    const starsFilters = this.pickQueryList(query, ["stars", "rating", "rate"])
      .map((entry) => this.toNumber(entry, NaN))
      .filter((entry) => Number.isFinite(entry))
      .map((entry) => Math.min(5, Math.max(0, Math.round(entry))));
    const replyRaw = this.pickQueryValue(query, [
      "reply",
      "has_reply",
      "replied",
    ]).toLowerCase();
    const publishRaw = this.pickQueryValue(query, [
      "publish",
      "published",
      "is_published",
    ]).toLowerCase();
    const startDateRaw = this.pickQueryValue(query, [
      "start_date",
      "created_from",
      "from_date",
    ]);
    const endDateRaw = this.pickQueryValue(query, [
      "end_date",
      "created_to",
      "to_date",
    ]);
    ...
    if (productFilters.length > 0) {
      const productSet = new Set(
        productFilters.map((entry) => String(entry).trim()),
      );
      filtered = filtered.filter((entry: any) =>
        productSet.has(String(entry?.product_id || "").trim()),
      );
    }

    if (blogFilters.length > 0) {
      const blogSet = new Set(blogFilters.map((entry) => String(entry).trim()));
      filtered = filtered.filter((entry: any) =>
        blogSet.has(String(entry?.blog_id || "").trim()),
      );
    }

    if (customerFilters.length > 0) {
      filtered = filtered.filter((entry: any) => {
        const customerId = String(entry?.customer?.id || "")
          .trim()
          .toLowerCase();
        const customerEmail = this.pickLocalizedText(
          entry?.customer?.email || entry?.customer_email || "",
        ).toLowerCase();
        const customerName = this.pickLocalizedText(
          entry?.customer?.name || entry?.customer_name || "",
        ).toLowerCase();
        return customerFilters.some(
          (token) =>
            token === customerId ||
            token === customerEmail ||
    ...
    if (starsFilters.length > 0) {
      const starsSet = new Set(starsFilters);
      filtered = filtered.filter((entry: any) =>
        starsSet.has(
          Math.round(this.toNumber(entry?.stars ?? entry?.rating, -1)),
        ),
      );
    }

    if (replyRaw === "true" || replyRaw === "1" || replyRaw === "yes") {
      filtered = filtered.filter((entry: any) =>
        Boolean(
          this.pickLocalizedText(entry?.reply || entry?.answer || "").trim(),
        ),
      );
    } else if (replyRaw === "false" || replyRaw === "0" || replyRaw === "no") {
      filtered = filtered.filter(
        (entry: any) =>
          !this.pickLocalizedText(entry?.reply || entry?.answer || "").trim(),
      );
    ...
    if (publishRaw === "true" || publishRaw === "1" || publishRaw === "yes") {
      filtered = filtered.filter((entry: any) => entry?.is_published !== false);
    } else if (
      publishRaw === "false" ||
      publishRaw === "0" ||
      publishRaw === "no"
    ...
    if (keyword) {
      filtered = filtered.filter((entry: any) => {
        const haystack = [
          ...
          .map((value) => this.pickLocalizedText(value).toLowerCase())
          .join(" ");
    ...
    if (startDate && Number.isFinite(startDate.getTime())) {
      ...
      filtered = filtered.filter((entry: any) => {
        const createdTs = new Date(String(entry?.created_at || "")).getTime();
    ...
    if (endDate && Number.isFinite(endDate.getTime())) {
      ...
      filtered = filtered.filter((entry: any) => {
        const createdTs = new Date(String(entry?.created_at || "")).getTime();
        return Number.isFinite(createdTs) && createdTs <= maxTs;
      });
    }
  ...
  public async getReview(storeId: string, reviewId: string) {
    ...
    const review = (reviews || []).find(
      (entry: any) => String(entry?.id || "") === String(reviewId),
    );
  ...
  public async createReview(storeId: string, data: any) {
    ...
    payload.product_id = await this.resolveFeedbackProductId(
      storeId,
      payload.product_id,
    );
  ...
  public async updateReview(storeId: string, reviewId: string, data: any) {
    ...
    const currentFallback =
      currentPrimary ||
      (await this.storeLogic.getDataEntity(
        storeId,
        "product_review",
        reviewId,
      ));
    ...
    const payload = this.normalizeReviewPayload(
      { ...(currentFallback || {}), ...(data || {}) },
      reviewId,
    );
    payload.product_id = await this.resolveFeedbackProductId(
      storeId,
      payload.product_id || previous.product_id,
    );
  ...
  public async getQuestions(storeId: string, productId?: string) {
    ...
    const filtered = normalizedProductId
      ? normalized.filter(
          (entry: any) => entry.product_id === normalizedProductId,
        )
      : normalized;
  ...
  public async getQuestion(storeId: string, questionId: string) {
    ...
    const question = (questions || []).find(
      (entry: any) => String(entry?.id || "") === String(questionId),
    );
    if (!question) return null;
    const normalized = this.normalizeQuestionPayload(
      question,
      String(question.id),
    );
    return this.wrapResponse(normalized);
  ...
  public async deleteQuestion(storeId: string, questionId: string) {
    ...
    await this.storeLogic.deleteDataEntity(
      storeId,
      "product_question",
      questionId,
    );
  ...
  public async getMenus(storeId: string, type: string) {
    const menuType =
      String(type || "header")
        .trim()
        .toLowerCase() || "header";
    const storedMenu = await this.storeLogic.getDataEntity(
      storeId,
      "menu",
      menuType,
    );
  ...
  private normalizeStaticPagePayload(data: any, key: string) {
    ...
    const url = this.normalizeEntityUrl(base?.url, `/pages/${slug || key}`, [
      "/page",
      "/pages",
    ]);
  ...
  public async getStaticPages(storeId: string) {
    ...
    const normalized = (pages || []).map((page: any) =>
      this.normalizeStaticPagePayload(
        page,
        String(page?.id || page?.slug || this.generateEntityId("page")),
      ),
    );
  ...
  public async getBlogCategories(storeId: string) {
    const categories = await this.getBlogCategoriesRaw(storeId);
    const normalized = categories.map((category: any) =>
      this.normalizeBlogCategoryPayload(
        category,
        String(category?.id || this.generateEntityId("blog_category")),
      ),
    );
    return this.wrapResponse(normalized);
  }
  ...
  public async updateBlogCategory(
    ...
    const payload = this.normalizeBlogCategoryPayload(
      { ...(current || {}), ...(data || {}) },
      categoryId,
    );
    await this.storeLogic.upsertDataEntity(
  ...
  public async deleteBlogCategory(storeId: string, categoryId: string) {
    ...
    for (const article of articles) {
      const articleId = String(
        article?.id || this.generateEntityId("blog_article"),
      );
      const payload = await this.normalizeBlogArticlePayload(
        storeId,
        {
          ...article,
          category_id:
            String(article?.category_id || article?.categoryId || "") === id
              ? ""
              : article?.category_id || article?.categoryId || "",
          category:
            String(article?.category?.id || "") === id
              ? undefined
              : article?.category,
        },
        articleId,
  ...
  public async getBlogArticles(
    ...
    const normalized = await Promise.all(
      (articles || []).map((article: any) =>
        this.normalizeBlogArticlePayload(
          storeId,
          article,
          String(article?.id || this.generateEntityId("blog_article")),
        ),
      ),
    );
    ...
    const categoryId =
      this.pickQueryValue(query, ["category_id", "categoryId", "category"]) ||
      (source.includes("category") ? sourceValue : "");
    const search = this.pickQueryValue(query, [
      "search",
      "q",
      "keyword",
    ...
    const published = this.pickQueryValue(query, [
      "published",
      "is_published",
    ]).toLowerCase();
    const page = this.pickQueryInt(
    ...
    if (categoryId) {
      ...
      filtered = filtered.filter((article: any) => {
        const refs =
          this.collectBlogArticleCategoryReferenceCandidates(article);
        return this.referencesIntersect(target, refs);
      });
    }
    ...
    if (published === "true" || published === "1") {
      filtered = filtered.filter(
        (article: any) => article?.is_published !== false,
      );
    }
    ...
    if (sort === "oldest" || sort === "created_asc" || sort === "created-asc") {
      ...
      filtered = filtered.sort((a, b) =>
        this.pickLocalizedText(a?.title).localeCompare(
          this.pickLocalizedText(b?.title),
          "ar",
        ),
      );
    } else {
  ...
  public async getBlogArticle(storeId: string, articleId: string) {
    ...
    const normalized = await this.normalizeBlogArticlePayload(
      storeId,
      article,
      String(article.id),
    );
  ...
  public async updateBlogArticle(
    storeId: string,
    articleId: string,
    data: any,
  ) {
  ...
  public async updateStaticPage(storeId: string, pageId: string, data: any) {
    ...
    const payload = this.normalizeStaticPagePayload(
      { ...(current || {}), ...(data || {}) },
      pageId,
    );
  ...
  public async getThemeSettings(storeId: string) {
    ...
    try {
      ...
      console.error(
        "[SIMULATOR] Failed to load dynamic theme settings via provider:",
        e,
      );
      // Dynamic fallback
      settings = [
  ...
  public async getThemeComponents(storeId: string) {
    ...
    try {
      if (store.themeId) {
        ...
        const blogArticles = [
          ...
          return {
            ...
            slug,
            name: this.pickLocalizedText(
              article?.name || article?.title || slug,
            ),
            title: this.pickLocalizedText(
              article?.title || article?.name || slug,
            ),
        ...
        const blogCategories = [
          ...
          return {
            ...
            title: this.pickLocalizedText(
              category?.title || category?.name || slug,
            ),
            url: this.pickLocalizedText(
              category?.url || `/blog/categories/${slug}`,
            ),
          };
        });
        ...
        const sources = {
          ...
          questions,
          offers_link: [
            { id: "offers_link", name: "التخفيضات", url: "/offers" },
          ],
          brands_link: [
            { id: "brands_link", name: "الماركات التجارية", url: "/brands" },
          ],
      ...
      console.error(
        "[SIMULATOR] Failed to load theme components via provider:",
        e,
      );
    }
