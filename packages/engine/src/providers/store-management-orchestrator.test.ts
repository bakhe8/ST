...
describe('StoreManagementOrchestrator', () => {
    it('validates settings payload before update', async () => {
        const orchestrator = new StoreManagementOrchestrator(
            { listStores: vi.fn(), createStore: vi.fn(), cloneStore: vi.fn(), updateStore: vi.fn(), deleteStore: vi.fn() } as any,
            { promoteToMaster: vi.fn(), inheritFrom: vi.fn(), updateSettings: vi.fn(), clearDataEntities: vi.fn() } as any,
    ...
    it('allows binding a theme when capability gate returns fail and exposes diagnostics in observe mode', async () => {
        const storeFactory = {
            ...
            createStore: vi.fn(),
            cloneStore: vi.fn(),
            updateStore: vi.fn().mockResolvedValue({ id: 'store-1', themeId: 'theme-failing' }),
            deleteStore: vi.fn()
        } as any;

        const storeLogic = {
            getStore: vi.fn().mockResolvedValue({ id: 'store-1', themeId: 'theme-a', themeVersionId: 'ver-a' }),
            promoteToMaster: vi.fn(),
            inheritFrom: vi.fn(),
            updateSettings: vi.fn(),
                ...
                versions: [
                    {
                        ...
                        version: '1.0.0',
                        contractJson: JSON.stringify({ name: 'Failing Theme', version: '1.0.0', components: [] })
                    }
                ]
            })
        } as any;
    ...
    it('allows binding a theme when capability gate is warning and continues update', async () => {
        const storeFactory = {
            ...
            createStore: vi.fn(),
            cloneStore: vi.fn(),
            updateStore: vi.fn().mockResolvedValue({ id: 'store-1', themeId: 'theme-warning' }),
            deleteStore: vi.fn()
        } as any;
                ...
                versions: [
                    {
                        ...
                        version: '1.0.0',
                        contractJson: JSON.stringify({ name: 'Warning Theme', version: '1.0.0', components: [{ path: 'home.slider' }] })
                    }
                ]
            })
        } as any;
