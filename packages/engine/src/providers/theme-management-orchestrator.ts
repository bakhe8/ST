...
export class ThemeManagementOrchestrator {
    ...
    public async discoverThemes() {
        ...
        const registeredIds = new Set(registeredThemes.map((theme: any) => theme.id));
        ...
        const discovery = await Promise.all(localFolders.map(async (folder) => {
            ...
            const compatibility = await this.sallaValidator.validateTheme(themePath, schema);
            const componentCapability = this.sallaValidator.evaluateThemeComponentCapability(schema);
            const anchorProbe = await this.sallaValidator.evaluateThemeAnchorProbe(themePath, schema);
    ...
    public async syncThemes(): Promise<ThemeActionResult<{
        synced: number;
        capabilityGate: {
            overallStatus: 'pass' | 'warning' | 'fail';
        ...
        try {
            ...
            const overallStatus = this.resolveAggregateCapabilityStatus(capabilityReports);
    ...
    private resolveAggregateCapabilityStatus(reports: ThemeCapabilitySnapshot[]): 'pass' | 'warning' | 'fail' {
