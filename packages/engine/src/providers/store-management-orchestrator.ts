...
type ThemeAdmissionDiagnostics = {
  ...
  capability: Pick<
    ThemeComponentCapabilityReport,
    "overallStatus" | "missingCorePages"
  >;
  anchorProbe: Pick<
    ThemeAnchorProbeReport,
    "overallStatus" | "missingAnchorPoints"
  > | null;
};
...
export class StoreManagementOrchestrator {
  ...
  public async promoteStore(storeId: string): Promise<StoreActionResult<any>> {
    try {
      ...
      return {
        ok: false,
        status: 500,
        message: error?.message || "Promote failed",
      };
    }
  }

  public async inheritStore(
    storeId: string,
    parentStoreId: unknown,
  ): Promise<StoreActionResult<any>> {
    try {
      const store = await this.storeLogic.inheritFrom(
        storeId,
        parentStoreId as string | null,
      );
      return { ok: true, status: 200, data: store };
    } catch (error: any) {
      return {
        ok: false,
        status: 500,
        message: error?.message || "Inherit failed",
      };
    }
  }
  ...
  public async seedStore(
    ...
    try {
      ...
      const result = await this.seeder.seedStoreData(
        storeId,
        count,
        undefined,
        {
  ...
  public async syncStore(
    ...
    try {
      const result = await this.synchronizer.syncStoreData(
        storeId,
        normalizedStoreUrl,
      );
      return { ok: true, status: 200, data: result };
      ...
      return {
        ok: false,
        status: 500,
        message: `Synchronization failed: ${error?.message || "unknown"}`,
      };
    }
  }
  ...
  public async clearStoreData(
    storeId: string,
  ): Promise<StoreActionResult<{ message: string }>> {
    try {
  ...
  public async deleteStore(
    ...
    try {
      ...
      return {
        ok: true,
        status: 200,
        data: { message: "Store and all associated data deleted" },
      };
    } catch (error: any) {
      return {
        ok: false,
  ...
  private async validateThemeAdmission(
    ...
    const source = payload && typeof payload === "object" ? payload : {};
    const hasThemeMutation =
      Object.prototype.hasOwnProperty.call(source, "themeId") ||
    ...
    const themeId =
      asString(source.themeId).trim() ||
      asString((store as any)?.themeId).trim();
    const themeVersionId =
      asString(source.themeVersionId).trim() ||
      asString((store as any)?.themeVersionId).trim();
    ...
    const versions = Array.isArray((theme as any)?.versions)
      ? (theme as any).versions
      : [];
    const selectedVersion =
      versions.find(
        (version: any) => asString(version?.id).trim() === themeVersionId,
      ) ||
      versions.find(
        (version: any) => asString(version?.version).trim() === themeVersionId,
      );
    ...
    if (
      typeof (this.sallaValidator as any).evaluateThemeAnchorProbe ===
      "function"
    ) {
      try {
        const themeRootPath = asString(selectedVersion?.fsPath).trim();
        if (themeRootPath) {
          anchorProbe = await this.sallaValidator.evaluateThemeAnchorProbe(
            themeRootPath,
            schema,
          );
        }
        ...
        console.warn(
          "[ThemeAdmission] anchor probe failed (observe mode):",
          error,
        );
      }
    }
    ...
    const overallStatus = resolveCombinedAdmissionStatus(
      capability.overallStatus,
      anchorStatus,
    );
    ...
    if (overallStatus !== "pass") {
      console.warn("[ThemeAdmission][observe]", {
        ...
        themeId,
        themeVersionId:
          asString(selectedVersion?.id).trim() ||
          asString(selectedVersion?.version).trim(),
        overallStatus,
