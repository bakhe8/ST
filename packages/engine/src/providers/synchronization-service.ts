...
export class SynchronizationService {
    ...
    public async syncStoreData(storeId: string, storeUrl: string, tx?: any) {
        ...
        // 2. Fetch Categories
        const categories = await this.fetchSallaData(`${info.baseUrl}/api/v1/categories`);
        for (const category of categories) {
            await this.simulationLogic.createDataEntity(storeId, 'category', category, tx);
        }

        // 3. Fetch Products
        const products = await this.fetchSallaData(`${info.baseUrl}/api/v1/products`);
        for (const product of products) {
            // Fetch variants and tags for each product
            const [variants, tags] = await Promise.all([
                this.fetchSallaData(`${info.baseUrl}/api/v1/products/${product.id}/variants`),
                this.fetchSallaData(`${info.baseUrl}/api/v1/products/${product.id}/tags`)
            ]);
        ...
        // 4. Fetch Store Info
        const storeInfoResults = await this.fetchSallaData(`${info.baseUrl}/api/v1/store/info`);
        ...
        // 5. Fetch Branches
        const branches = await this.fetchSallaData(`${info.baseUrl}/api/v1/branches`);
        ...
        // 7. Fetch Shipping
        const shipping = await this.fetchSallaData(`${info.baseUrl}/api/v1/shipping`);
        storeInfo.shipping = shipping;

        // 8. Fetch Currencies
        const currencies = await this.fetchSallaData(`${info.baseUrl}/api/v1/currencies`);
        storeInfo.currencies = currencies;

        // 9. Fetch Languages
        const languages = await this.fetchSallaData(`${info.baseUrl}/api/v1/languages`);
        storeInfo.languages = languages;

        await this.simulationLogic.createDataEntity(storeId, 'store', storeInfo, tx);
        ...
        const templates = await this.fetchSallaData(`${info.baseUrl}/api/v1/products/options/templates`);
        for (const template of templates) {
            await (this.simulationLogic as any).createDataEntity(storeId, 'optionTemplate', template, tx);
        }

        // 12. Fetch Special Offers
        const offers = await this.fetchSallaData(`${info.baseUrl}/api/v1/marketing/special-offers`);
        for (const offer of offers) {
            await (this.simulationLogic as any).createDataEntity(storeId, 'specialOffer', offer, tx);
        }

        // 13. Fetch Affiliates
        const affiliates = await this.fetchSallaData(`${info.baseUrl}/api/v1/marketing/affiliates`);
        for (const affiliate of affiliates) {
            await (this.simulationLogic as any).createDataEntity(storeId, 'affiliate', affiliate, tx);
        }
    ...
    private extractStoreInfo(url: string): { identifier: string, baseUrl: string } | null {
        try {
            ...
            if (hostname === 'localhost' || hostname === '127.0.0.1') {
                ...
                return { identifier, baseUrl: `${parsed.protocol}//${parsed.host}/${identifier}` };
            }
            ...
            if (hostname === 'salla.sa') {
                const parts = parsed.pathname.split('/').filter(p => p);
                if (parts[0]) {
                    return { identifier: parts[0], baseUrl: `https://salla.sa/${parts[0]}` };
                }
            }
                ...
                res.on('end', () => {
                    if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
                        try {
                            ...
                            console.error(`[Sync] Response body (truncated): ${data.substring(0, 200)}`);
                            resolve([]);
                        }
                    } else {
                        console.warn(`[Sync] Non-200 Response for ${url}: ${res.statusCode}`);
                        resolve([]);
                    }
                });
            }).on('error', (err) => {
