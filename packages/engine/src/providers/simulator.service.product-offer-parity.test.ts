...
function createService(entitiesByType: Record<string, any[]>) {
    const storeLogic = {
        getDataEntities: vi.fn().mockImplementation(async (_storeId: string, type: string) => entitiesByType[type] || []),
    ...
    return {
        service: new SimulatorService(storeLogic, schemaService, themeRuntimeAdapter),
        storeLogic
    };
}
