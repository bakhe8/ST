...
export class WebhookService {
  ...
  public async dispatchEvent(
    ...
    console.log(
      `[Webhook] Dispatching '${event}' to ${activeSubs.length} subscriptions`,
    );
  ...
  private async sendWebhook(
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = protocol.request(options, (res) => {
        console.log(
          `[Webhook] Sent to ${subscription.url} - Status: ${res.statusCode}`,
        );
        resolve();
      });
      ...
      req.on("error", (e) => {
        console.error(
          `[Webhook] Error sending to ${subscription.url}:`,
          e.message,
        );
