...
const THEME_COMPONENT_PAGE_RULES: ThemeComponentPageRule[] = [
    {
        ...
        prefixes: ['home'],
        core: true,
        templateCandidates: ['src/views/pages/index.twig', 'src/views/pages/home.twig'],
        bindingTokens: ['home']
    },
    {
        id: 'product-list',
    ...
    },
    {
        ...
        core: false,
        templateCandidates: ['src/views/pages/product/single.twig', 'src/views/pages/product.twig'],
    ...
    },
    {
        ...
        prefixes: ['category'],
        core: true,
        templateCandidates: ['src/views/pages/category/index.twig', 'src/views/pages/product/index.twig'],
    ...
    },
    {
        ...
        prefixes: ['cart'],
        core: false,
        templateCandidates: ['src/views/pages/cart.twig', 'src/views/pages/checkout.twig'],
        bindingTokens: ['cart']
    },
    {
        id: 'profile',
    ...
    },
    {
        ...
        core: true,
        templateCandidates: ['src/views/pages/blog/index.twig', 'src/views/pages/blog.twig'],
        bindingTokens: ['blog']
    },
    {
        id: 'blog-single',
    ...
    },
    {
        ...
        core: true,
        templateCandidates: ['src/views/pages/brands/index.twig', 'src/views/pages/brands.twig'],
    ...
    },
    {
        ...
        prefixes: ['thank-you'],
        core: false,
        templateCandidates: ['src/views/pages/thank-you.twig', 'src/views/pages/thank_you.twig'],
    ...
    },
    {
        ...
        core: false,
        templateCandidates: ['src/views/pages/page-single.twig', 'src/views/pages/page/single.twig'],
...
export class SallaValidator {
    ...
    public evaluateThemeComponentCapability(schema: TwilightSchema): ThemeComponentCapabilityReport {
        ...
        const pages = THEME_COMPONENT_PAGE_RULES.map((pageRule) => {
            ...
            const matchedPaths = Array.from(new Set(matched.map((entry) => entry.path)));
            const matchedComponentKeys = Array.from(new Set(matched.map((entry) => entry.key)));
        ...
        const supportedPageIds = pages.filter((page) => page.supported).map((page) => page.id);
        const missingCorePages = pages.filter((page) => page.core && !page.supported).map((page) => page.id);
        ...
        const coveredCorePages = pages.filter((page) => page.core && page.supported).length;
        const matchedComponents = pages.reduce((sum, page) => sum + page.matchedComponents, 0);
    ...
    public async evaluateThemeAnchorProbe(themePath: string, schema: TwilightSchema): Promise<ThemeAnchorProbeReport> {
        const components = this.normalizeThemeComponents(schema);
        ...
        for (const pageRule of THEME_COMPONENT_PAGE_RULES) {
            ...
            const templatePath = await this.resolveFirstExistingTemplate(themePath, pageRule.templateCandidates);
            const pageAnchors = new Set<string>(layoutAnchors);
            if (templatePath) {
                const templateContent = await this.readTextSafe(path.join(themePath, templatePath));
                this.extractComponentAnchors(templateContent).forEach((anchor) => pageAnchors.add(anchor));
            }
            ...
            for (const component of pageComponents) {
                const covered = Array.from(pageAnchors).some((anchor) =>
                    this.anchorCoversComponent(anchor, component.path, pageRule.bindingTokens)
                );
                if (covered) {
        ...
        pages.forEach((page) => page.detectedAnchors.forEach((anchor) => allAnchors.add(anchor)));
        ...
        const coreMissing = pages.some((page) => page.core && page.missingAnchorPoints.length > 0);
        const pageMissingCount = pages.filter((page) => page.missingAnchorPoints.length > 0).length;
    ...
    private normalizeThemeComponents(schema: TwilightSchema): NormalizedThemeComponent[] {
        if (!Array.isArray(schema?.components)) return [];
        return schema.components
            .map((component: any, index: number) => {
                const path = this.normalizeComponentPath(component?.path);
                if (!path) return null;
                const keyCandidate = typeof component?.key === 'string' ? component.key.trim() : '';
    ...
    private pathMatchesPrefix(componentPath: string, prefixValue: string): boolean {
        const prefix = this.normalizeComponentPath(prefixValue);
        if (!prefix) return false;
        if (componentPath === prefix) return true;
        return componentPath.startsWith(`${prefix}.`) || componentPath.startsWith(`${prefix}/`);
    }

    private componentMatchesPageRule(componentPathValue: string, pageRule: ThemeComponentPageRule): boolean {
        const componentPath = this.normalizeComponentPath(componentPathValue);
        if (!componentPath) return false;

        const included = pageRule.prefixes.some((prefix) => this.pathMatchesPrefix(componentPath, prefix));
    ...
    private async resolveFirstExistingTemplate(themePath: string, candidates: string[]): Promise<string | null> {
    ...
    private normalizeAnchorToken(tokenValue: unknown): string {
        ...
        const normalized = this.normalizeComponentPath(withoutWrappers).replace(/\//g, '.');
    ...
    private anchorRefersToComponent(anchorValue: string, componentPathValue: string): boolean {
        const anchor = this.normalizeAnchorToken(anchorValue);
        const componentPath = this.normalizeComponentPath(componentPathValue).replace(/\//g, '.');
        if (!anchor || !componentPath) return false;
        if (anchor === componentPath) return true;
        return componentPath.startsWith(`${anchor}.`) || anchor.startsWith(`${componentPath}.`);
    }

    private anchorCoversComponent(anchorValue: string, componentPathValue: string, bindingTokens: string[]): boolean {
        const anchor = this.normalizeAnchorToken(anchorValue);
        const componentPath = this.normalizeComponentPath(componentPathValue).replace(/\//g, '.');
        if (!anchor || !componentPath) return false;
    ...
    private async collectLayoutAnchors(themePath: string): Promise<Set<string>> {
        ...
        for (const entry of entries) {
            ...
            this.extractComponentAnchors(content).forEach((anchor) => anchors.add(anchor));
        }
        return anchors;
    }
