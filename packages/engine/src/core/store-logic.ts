...
export class StoreLogic {
  ...
  public async updateThemeSettings(
    storeId: string,
    ...
    if (existing.page_compositions || incoming.page_compositions) {
      merged.page_compositions = {
        ...(existing.page_compositions &&
        typeof existing.page_compositions === "object"
          ? existing.page_compositions
          : {}),
        ...(incoming.page_compositions &&
        typeof incoming.page_compositions === "object"
          ? incoming.page_compositions
          : {}),
      };
    }
  ...
  public async upsertDataEntity(
    ...
    return this.dataEntityRepo.upsertByEntityKey(
      storeId,
      type,
      key,
      payload,
  ...
  public async getDataEntities(storeId: string, type: string, tx?: any) {
    ...
    const localEntities = await this.dataEntityRepo.getByStoreAndType(
      storeId,
      type,
      tx,
    );
    ...
    if (store?.parentStoreId) {
      console.log(
        `[SimulationLogic] Inheriting ${type} from parent: ${store.parentStoreId}`,
      );
      const parentEntities = await this.getDataEntities(
        store.parentStoreId,
        type,
        tx,
      );
  ...
  public async inheritFrom(
    ...
    if (parentStoreId) {
      ...
      if (hasCycle) {
        throw new Error(
          `Circular inheritance detected: Store ${storeId} is already a parent (directly or indirectly) of ${parentStoreId}.`,
        );
      }
    }

    console.log(
      `[SimulationLogic] Setting parent for store ${storeId} -> ${parentStoreId}`,
    );
