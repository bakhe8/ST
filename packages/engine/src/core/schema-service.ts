...
export class SchemaService {
    ...
    public async initialize() {
        ...
        try {
            ...
            console.log(`[SchemaService] Initialized with ${Object.keys(models).length} models, ${Object.keys(filters).length} filters, and ${Object.keys(functions).length} functions.`);
        } catch (error) {
            console.error('[SchemaService] Failed to initialize metadata:', error);
            // Provide empty defaults on error to avoid crashing
            this.metadata = { filters: {}, functions: {}, components: [], hooks: [], models: {} };
        }
    }
    ...
    private async loadJson(filePath: string): Promise<any> {
        try {
            ...
            console.warn(`[SchemaService] Failed to load JSON from ${filePath}:`, error);
            return {};
        }
    }

    public getMetadata(): SchemaMetadata {
        return this.metadata || { filters: {}, functions: {}, components: [], hooks: [], models: {} };
    }
