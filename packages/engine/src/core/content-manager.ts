import { IDataEntityRepository, ICollectionRepository, IDataBindingRepository, Product, Collection, DataBinding } from '@vtdr/contracts';
...
export class ContentManager {
    ...
    public async getProduct(storeId: string, productId: string): Promise<any | null> {
    ...
    public async createCollection(storeId: string, collection: any) {
        ...
        if (collection.source === 'manual' && collection.items.length > 0) {
            await this.collectionRepo.addItems(dbCollection.id, collection.items.map((productId: string, index: number) => ({
                entityId: productId,
                sortOrder: index
            })));
        }
