    ...
    it('hydrates cart, orders, and checkout entities into preview context', async () => {
        ...
        const storeLogic = {
            getDataEntity: async (_storeId: string, type: string) => {
                ...
                if (type === 'checkout_session') return { id: 'checkout-default', step: 'address' };
                return null;
            },
...
describe('renderPreviewWithFallback', () => {
    it('retries with index template when home render fails with home.twig error', async () => {
        ...
        const renderer = {
            renderPage: async () => {
                call += 1;
                return call === 1 ? 'Render Error home.twig missing' : '<html>ok-index</html>';
            }
        } as any;
    ...
    it('falls back to page-single for unsupported render targets', async () => {
        ...
        const renderer = {
            renderPage: async () => {
                call += 1;
                return call === 1 ? 'Renderer Error: missing brands/index.twig' : '<html>page-single</html>';
            }
        } as any;
