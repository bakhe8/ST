import { ThemeMetadata, ThemeVersion, TwilightSchema, IThemeRepository } from '@vtdr/contracts';
...
export class ThemeRegistry {
    ...
    public async addVersion(themeId: string, versionNumber: string, fsPath: string, schema: TwilightSchema) {
        return this.themeRepo.addVersion(themeId, versionNumber, fsPath, schema);
    }

    public async syncTheme(metadata: ThemeMetadata, schema: TwilightSchema, fsPath: string) {
        const theme = await this.registerTheme(metadata);

        // Check if version exists
        const existingVersion = await this.themeRepo.findVersion(theme.id, metadata.version);
