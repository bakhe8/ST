...
const normalizePathToken = (value: unknown): string => {
    const raw = Array.isArray(value) ? value.map((entry) => asString(entry)).join('/') : asString(value);
    return value
        ? raw
        .trim()
        .replace(/\\/g, '/')
...
const normalizeEntityUrl = (candidate: unknown, fallbackPath: string, genericPaths: string[] = []): string => {
...
const resolveEntityByRef = (items: EntityRecord[], rawRef?: string): EntityRecord | null => {
...
const groupBrandsForTemplate = (brands: EntityRecord[]): Record<string, EntityRecord[]> => {
...
const normalizeStoreForRuntime = (store: EntityRecord): EntityRecord => {
    ...
    const contacts = (store.contacts && typeof store.contacts === 'object')
        ? (store.contacts as EntityRecord)
        : {};
    const social = (store.social && typeof store.social === 'object')
        ? (store.social as EntityRecord)
        : {};
    const settings = (store.settings && typeof store.settings === 'object')
        ? (store.settings as EntityRecord)
        : {};
    const cartSettings = (settings.cart && typeof settings.cart === 'object')
    ...
    return {
        ...
        currency,
        currencies: Array.isArray(store.currencies) && store.currencies.length > 0
            ? store.currencies
            : [
                {
                    code: currency,
        ...
        languages: Array.isArray(store.languages) && store.languages.length > 0
            ? store.languages
            : [
                {
        ...
        logo: sanitizePublicImageUrl(store.logo || store.avatar || (store.branding as EntityRecord | undefined)?.logo),
        icon: sanitizePublicImageUrl(store.icon || store.avatar || (store.branding as EntityRecord | undefined)?.logo),
        contacts: {
            mobile: asString(contacts.mobile) || '+966500000000',
            email: asString(contacts.email) || 'support@salla.sa',
            whatsapp: asString(contacts.whatsapp || contacts.mobile) || '+966500000000'
        },
        social: {
...
const normalizeRuntimeUser = (store: EntityRecord, rawUser: unknown): EntityRecord => {
    const locale = asString(store.locale) || 'ar-SA';
    ...
    const existing = rawUser && typeof rawUser === 'object' ? (rawUser as EntityRecord) : {};
    const rawName = asString(existing.name) || 'ضيف المحاكي';
    const nameParts = rawName.split(/\s+/).filter(Boolean);
    const firstName = asString(existing.first_name) || nameParts[0] || 'ضيف';
    const lastName = asString(existing.last_name) || (nameParts.length > 1 ? nameParts.slice(1).join(' ') : '');
    const inferredAuth = existing.type === 'user' || existing.is_authenticated === true;
    const userType = asString(existing.type) || (inferredAuth ? 'user' : 'guest');
    const isAuthenticated = typeof existing.is_authenticated === 'boolean'
    ...
    return {
        ...
        language: {
            code: asString((existing.language as EntityRecord | undefined)?.code || existing.language_code) || languageCode,
            dir: asString((existing.language as EntityRecord | undefined)?.dir) || languageDir
        },
        language_code: asString(existing.language_code) || languageCode,
        currency: {
            code: asString((existing.currency as EntityRecord | undefined)?.code) || currencyCode,
            symbol: asString((existing.currency as EntityRecord | undefined)?.symbol) || (currencyCode === 'SAR' ? 'ر.س' : currencyCode)
        },
...
export const resolvePreviewTarget = (rawPage: unknown, rawWildcardPath: string): ResolvedPreviewTarget => {
    ...
    if (wildcardPath) {
        ...
        if (first === 'products' || first === 'product') {
            if (segments.length >= 2) return { pageId: 'product/single', routePath: `/${normalizedPath}`, entityRef: last };
            return { pageId: 'product/index', routePath: `/${normalizedPath}` };
        }

        if (first === 'categories' || first === 'category') {
            if (segments.length >= 2) return { pageId: 'product/index', routePath: `/${normalizedPath}`, entityRef: last };
            return { pageId: 'product/index', routePath: `/${normalizedPath}` };
        }

        if (first === 'brands' || first === 'brand') {
            if (segments.length >= 2) return { pageId: 'brands/single', routePath: `/${normalizedPath}`, entityRef: last };
            return { pageId: 'brands/index', routePath: `/${normalizedPath}` };
        }

        if (first === 'blog') {
            if (segments.length === 1) return { pageId: 'blog/index', routePath: '/blog' };
            if ((second === 'category' || second === 'categories') && segments.length >= 3) {
                return { pageId: 'blog/index', routePath: `/${normalizedPath}`, collectionRef: last };
            }
            return { pageId: 'blog/single', routePath: `/${normalizedPath}`, entityRef: last };
        }

        if (first === 'pages' || first === 'page') {
            return { pageId: 'page-single', routePath: `/${normalizedPath}`, entityRef: last };
        }
        ...
        if (first === 'customer') {
            ...
            if (secondToken === 'orders') {
                ...
                if (thirdToken === 'index') {
                    return { pageId: 'customer/orders/index', routePath: `/${normalizedPath}` };
                }
            ...
            if (secondToken === 'wishlist' || secondToken === 'notifications' || secondToken === 'wallet') {
                return { pageId: `customer/${secondToken}`, routePath: `/${normalizedPath}` };
            }
            return {
        ...
        if (first === 'cart' || first === 'loyalty' || first === 'testimonials' || first === 'thank-you' || first === 'landing-page' || first === 'debug') {
    ...
    if (!queryToken || queryToken === 'home' || queryToken === 'index') return { pageId: 'index', routePath: '/' };
    if (queryToken === 'products' || queryToken === 'product') return { pageId: 'product/index', routePath: '/products' };
    if (queryToken === 'categories' || queryToken === 'category') return { pageId: 'product/index', routePath: '/categories' };
    if (queryToken === 'brands' || queryToken === 'brand') return { pageId: 'brands/index', routePath: '/brands' };
    if (queryToken === 'blog') return { pageId: 'blog/index', routePath: '/blog' };
    if (queryToken === 'checkout') return { pageId: 'checkout', routePath: '/checkout' };
    if (queryToken === 'customer' || queryToken === 'customer/profile') return { pageId: 'customer/profile', routePath: '/customer/profile' };
    if (queryToken === 'customer/orders') return { pageId: 'customer/orders/index', routePath: '/customer/orders' };
    if (queryToken === 'customer/wishlist') return { pageId: 'customer/wishlist', routePath: '/customer/wishlist' };
    if (queryToken === 'customer/notifications') return { pageId: 'customer/notifications', routePath: '/customer/notifications' };
    if (queryToken === 'customer/wallet') return { pageId: 'customer/wallet', routePath: '/customer/wallet' };
    if (queryToken === 'pages' || queryToken === 'page' || queryToken === 'page-single') return { pageId: 'page-single', routePath: '/pages' };
    ...
    return { pageId: pageFromQuery, routePath: pageFromQuery ? `/${pageFromQuery}` : '/' };
};

const normalizeProductOptionDetailsForTemplate = (option: EntityRecord, currency: string): EntityRecord[] => {
    const source = asArray(option.details).length > 0 ? asArray(option.details) : asArray(option.values);
    const optionId = asString(option.id) || 'option';

    return source.map((entry, index) => {
        const detailId = asString(entry.id) || `${optionId}-detail-${index + 1}`;
        const name = asString(entry.name || entry.label || entry.title) || `Value ${index + 1}`;
        const additionalPrice = readMoneyAmount(entry.additional_price ?? entry.price ?? entry.amount);
        const optionValue = asString(entry.option_value ?? entry.value ?? entry.image ?? entry.color);
        ...
        const fullName = asString(entry.full_name) || (additionalPrice > 0 ? `${name} (+${additionalPrice} ${currency})` : name);
...
const normalizeProductOptionsForTemplate = (options: EntityRecord[], currency: string): EntityRecord[] => {
    return options.map((option, index) => {
        ...
        return {
            ...
            visibility_condition:
                option.visibility_condition && typeof option.visibility_condition === 'object'
...
const normalizeProductDonationForTemplate = (product: EntityRecord, fallbackAmount: number): EntityRecord | undefined => {
    const donationSource =
        product.donation && typeof product.donation === 'object' && !Array.isArray(product.donation)
    ...
    const isDonation = typeof product.is_donation === 'boolean'
        ? product.is_donation
        : (asString(product.type).toLowerCase() === 'donating' || hasDonationSignal);
    ...
    const computedPercent = targetAmount > 0 ? Math.min(100, (collectedAmount / targetAmount) * 100) : 0;
...
const collectProductCategoryIdsForPreview = (product: EntityRecord): string[] => {
    return toUniqueStrings([
        ...
        asString((product.category as EntityRecord | undefined)?.id || product.category_id || product.categoryId)
    ]);
};
...
const normalizeOfferForTemplate = (offer: EntityRecord, defaultCurrency: string): EntityRecord => {
    const offerId = asString(offer.id) || 'offer-1';
    const offerTitle = asString(offer.title || offer.name) || 'عرض';
    const offerSlug = slugifyToken(offer.slug || offerTitle || offerId) || offerId;

    const normalizedCategories = asArray(offer.categories).map((entry) => normalizeCategoryForTemplate(entry));
    ...
    const normalizedProducts = productsSource.map((entry) => normalizeProductForTemplate(entry, defaultCurrency));
    ...
    return {
        ...
        url: normalizeEntityUrl(offer.url, `/offers/${offerSlug}`, ['/offer', '/offers']),
        image: sanitizePublicImageUrl(offer.image || (offer.cover as EntityRecord | undefined)?.url),
        categories: normalizedCategories.length > 0
            ? normalizedCategories
            : categoryIds.map((id) =>
                normalizeCategoryForTemplate({ id, name: id, slug: id, url: `/categories/${id}` })
            ),
        category_ids: categoryIds,
        products: normalizedProducts.length > 0
            ? normalizedProducts
            : productIds.map((id) =>
                normalizeProductForTemplate({ id, name: id, slug: id, url: `/products/${id}` }, defaultCurrency)
            ),
        product_ids: productIds,
        discount_type: asString(offer.discount_type || offer.type || 'percentage'),
        discount_value: readMoneyAmount(offer.discount_value ?? offer.discount ?? offer.value),
...
const resolveRelatedProductsForPreview = (targetProduct: EntityRecord, products: EntityRecord[], limit = 12): EntityRecord[] => {
    const productId = asString(targetProduct.id).trim();
    const targetCategoryIds = new Set(collectProductCategoryIdsForPreview(targetProduct));
    const targetBrandId = asString((targetProduct.brand as EntityRecord | undefined)?.id || targetProduct.brand_id).trim();

    const scored = products
        .filter((entry) => asString(entry.id).trim() !== productId)
        .map((entry) => {
            const categoryIds = collectProductCategoryIdsForPreview(entry);
            const sharedCategoryScore = categoryIds.reduce((sum, categoryId) => sum + (targetCategoryIds.has(categoryId) ? 1 : 0), 0);
            const brandId = asString((entry.brand as EntityRecord | undefined)?.id || entry.brand_id).trim();
    ...
    return scoped
        .sort((a, b) => {
            ...
            const featuredDelta = Number(Boolean((b.product as EntityRecord).is_featured)) - Number(Boolean((a.product as EntityRecord).is_featured));
            if (featuredDelta !== 0) return featuredDelta;
            return readMoneyAmount((a.product as EntityRecord).sale_price || (a.product as EntityRecord).price) -
                readMoneyAmount((b.product as EntityRecord).sale_price || (b.product as EntityRecord).price);
        })
...
const resolveOfferForProduct = (
    ...
    const requestedRef = normalizePathToken(requestedOfferRef || '').toLowerCase();
    if (requestedRef) {
        return resolveEntityByRef(offers, requestedRef);
    }

    const productId = asString(product.id).trim();
    const productCategoryIds = new Set(collectProductCategoryIdsForPreview(product));
...
const normalizeProductForTemplate = (product: EntityRecord, defaultCurrency: string): EntityRecord => {
    ...
    const normalizedImages = images.length > 0 ? images : [{ id: 'img-1', url: DEFAULT_IMAGE, alt: asString(product.name) || 'image' }];
    const price = readMoneyAmount(product.price);
    const regularPrice = readMoneyAmount(product.regular_price || product.regularPrice) || price;
    const salePrice = readMoneyAmount(product.sale_price || product.salePrice) || price;
    const variants = asArray(product.variants);
    const currency = asString(product.currency) || defaultCurrency;
    const normalizedOptions = normalizeProductOptionsForTemplate(asArray(product.options), currency);
    const donation = normalizeProductDonationForTemplate(product, salePrice || price || 1);
    const isDonation = typeof product.is_donation === 'boolean'
        ? product.is_donation
        : (asString(product.type).toLowerCase() === 'donating' || Boolean(donation));
    const productType = isDonation ? 'donating' : (asString(product.type) || 'product');
    ...
    const maxQuantity = isInfiniteQuantity
        ? (Number.isFinite(explicitMaxQuantity) && explicitMaxQuantity > 0 ? Math.floor(explicitMaxQuantity) : 99)
        : Math.max(
            1,
            Math.min(
                Number.isFinite(explicitMaxQuantity) && explicitMaxQuantity > 0 ? Math.floor(explicitMaxQuantity) : (quantityNumber || 1),
                quantityNumber || (Number.isFinite(explicitMaxQuantity) ? Math.floor(explicitMaxQuantity) : 1) || 1
            )
        );
    const explicitAvailability = typeof product.is_available === 'boolean' ? product.is_available : undefined;
    const rawStatus = asString(product.status).toLowerCase();
    let computedAvailability = explicitAvailability ?? (isInfiniteQuantity || quantityNumber > 0);
    let isOutOfStock =
        Boolean(product.is_out_of_stock) ||
        !computedAvailability ||
        (!isInfiniteQuantity && quantityNumber <= 0);
    let status = isOutOfStock
        ? (rawStatus === 'out-and-notify' || Boolean(product.notify_availability) ? 'out-and-notify' : 'out')
        : 'sale';
    if (rawStatus === 'hidden') {
        status = 'hidden';
    ...
    const soldQuantity = Math.max(0, Math.floor(Number(product.sold_quantity || product.sales_count || 0) || 0));
    const isHiddenQuantity = Boolean(product.is_hidden_quantity);
    const notifyAvailability = isOutOfStock && status === 'out-and-notify'
        ? (
            product.notify_availability && typeof product.notify_availability === 'object'
                ? product.notify_availability
                : (
                    status === 'out-and-notify'
                        ? { channels: ['sms', 'email'], subscribed: false, subscribed_options: [], options: true }
                        : undefined
                )
        )
        : undefined;
    ...
    const url = normalizeEntityUrl(
        product.url,
        `/products/${productSlug}`,
        ['/product', '/products']
    );
    const rawBrand = (product.brand && typeof product.brand === 'object')
        ? (product.brand as EntityRecord)
        : {};
    const brandId = asString(rawBrand.id || product.brand_id || product.brandId);
    const brandSlug = slugifyToken(rawBrand.slug || rawBrand.name || brandId) || brandId;
    const normalizedBrand = brandId
        ? {
            ...rawBrand,
            id: brandId,
            name: asString(rawBrand.name || rawBrand.title) || 'ماركة',
            logo: sanitizePublicImageUrl(rawBrand.logo || (rawBrand.image as EntityRecord | undefined)?.url),
            url: normalizeEntityUrl(rawBrand.url, `/brands/${brandSlug}`, ['/brand', '/brands'])
        }
        : undefined;
    const normalizedCategories = asArray(product.categories).map((entry) => {
        const record = entry && typeof entry === 'object'
            ? (entry as EntityRecord)
            : { id: entry };
        const categoryId = asString(record.id || record.category_id || record.value);
        const categoryName = asString(record.name || record.title) || 'تصنيف';
        const categorySlug = slugifyToken(record.slug || categoryName || categoryId) || categoryId;
        ...
        return {
            ...
            slug: categorySlug,
            url: normalizeEntityUrl(record.url, `/categories/${categorySlug}`, ['/category', '/categories']),
    ...
    const primaryCategory = normalizedCategories[0] || (
        categoryIds.length > 0
            ? {
                id: categoryIds[0],
                name: asString((product.category as EntityRecord | undefined)?.name) || 'تصنيف',
                title: asString((product.category as EntityRecord | undefined)?.title) || asString((product.category as EntityRecord | undefined)?.name) || 'تصنيف',
                slug: slugifyToken((product.category as EntityRecord | undefined)?.slug || categoryIds[0]) || categoryIds[0],
                ...
                image: sanitizePublicImageUrl((product.category as EntityRecord | undefined)?.image)
            } as EntityRecord
            : undefined
    );
    ...
    return {
        ...
        rating: (product.rating && typeof product.rating === 'object') ? product.rating : { stars: 0, count: 0 },
        comments: asArray(product.comments),
        questions: asArray(product.questions),
        questions_count: Number(product.questions_count || asArray(product.questions).length || 0),
        quantity,
        stock: quantity,
        ...
        can_show_sold: typeof product.can_show_sold === 'boolean' ? product.can_show_sold : soldQuantity > 0,
        has_options: typeof product.has_options === 'boolean' ? product.has_options : (variants.length > 0 || normalizedOptions.length > 0),
        is_on_sale: typeof product.is_on_sale === 'boolean' ? product.is_on_sale : salePrice < regularPrice,
        price,
        regular_price: regularPrice,
        sale_price: salePrice,
        base_currency_price: Number(product.base_currency_price || salePrice || price),
        add_to_cart_label: asString(product.add_to_cart_label) || (isOutOfStock ? 'نفد المخزون' : 'أضف للسلة'),
        currency,
        main_image: sanitizePublicImageUrl(product.main_image || normalizedImages[0]?.url),
        thumbnail: sanitizePublicImageUrl(product.thumbnail || normalizedImages[0]?.url),
        brand: normalizedBrand,
        brand_id: asString(product.brand_id || (normalizedBrand as EntityRecord | undefined)?.id),
        categories: normalizedCategories.length > 0 ? normalizedCategories : (primaryCategory ? [primaryCategory] : []),
        category_ids: categoryIds,
        category: primaryCategory,
        category_id: asString(product.category_id || (primaryCategory as EntityRecord | undefined)?.id),
        has_read_more: typeof product.has_read_more === 'boolean' ? product.has_read_more : false,
        can_add_note: typeof product.can_add_note === 'boolean' ? product.can_add_note : true,
        can_upload_file: typeof product.can_upload_file === 'boolean' ? product.can_upload_file : false,
        has_custom_form: typeof product.has_custom_form === 'boolean'
            ? product.has_custom_form
            : Boolean(product.has_custome_form),
        has_custome_form: typeof product.has_custome_form === 'boolean'
        ...
        max_amount_donating: Number(product.max_amount_donating || (donation as EntityRecord | undefined)?.target_amount || 0) || undefined,
        is_require_shipping: typeof product.is_require_shipping === 'boolean'
...
const normalizeBrandForTemplate = (brand: EntityRecord): EntityRecord => {
    ...
    return {
        ...
        logo: sanitizePublicImageUrl(brand.logo || (brand.image as EntityRecord | undefined)?.url),
...
const normalizeBlogCategoryForTemplate = (category: EntityRecord): EntityRecord => {
...
const normalizeArticleForTemplate = (article: EntityRecord): EntityRecord => {
    const imageUrl = sanitizePublicImageUrl((article.image as EntityRecord | undefined)?.url || article.image);
    const imageAlt = asString((article.image as EntityRecord | undefined)?.alt || article.title) || 'article image';
    const articleId = asString(article.id) || asString(article.key) || 'blog-article';
    ...
    return {
        ...
        body: asString(article.body || article.content) || '<p>محتوى المقالة غير متوفر.</p>',
        url,
...
const normalizeStaticPageForTemplate = (page: EntityRecord): EntityRecord => {
    ...
    return {
        ...
        title,
        slug,
        content: asString(page.content || page.description) || '<p>هذه صفحة افتراضية من بيئة المعاينة.</p>',
        url: normalizeEntityUrl(
            page.url,
            `/pages/${slug}`,
            ['/page', '/pages']
...
const normalizeCartItemForTemplate = (item: EntityRecord, currency: string): EntityRecord => {
    const unitPrice = readMoneyAmount(item.price || item.unit_price || item.sale_price);
    const productPrice = readMoneyAmount(item.product_price || item.regular_price) || unitPrice;
    ...
    const productSlug = slugifyToken(item.slug || productName || productId) || productId;
    const url = normalizeEntityUrl(item.url, `/products/${productSlug}`, ['/product', '/products']);
    const productImage = sanitizePublicImageUrl(item.product_image || (item.image as EntityRecord | undefined)?.url || item.image);
    ...
    return {
        ...
        detailed_offers: Array.isArray(item.detailed_offers) ? item.detailed_offers : [],
        offer: (item.offer && typeof item.offer === 'object') ? item.offer : undefined,
        has_discount: Boolean(item.has_discount ?? unitPrice < productPrice),
        is_on_sale: Boolean(item.is_on_sale ?? unitPrice < originalPrice),
        special_price: readMoneyAmount(item.special_price || Math.max(0, productPrice - unitPrice)),
...
const normalizeCartForTemplate = (cart: EntityRecord, currency: string): EntityRecord => {
    const items = asArray(cart.items).map((entry) => normalizeCartItemForTemplate(entry, currency));
    const count = items.reduce((sum, entry) => sum + Number(entry.quantity || 0), 0);
    const subTotal = readMoneyAmount(cart.sub_total || (cart.totals as EntityRecord | undefined)?.subtotal);
    ...
    const total = readMoneyAmount(cart.total) || Math.max(0, subTotal + optionsTotal - discount + taxAmount + shippingCost);
    const freeShippingBar = (cart.free_shipping_bar && typeof cart.free_shipping_bar === 'object')
    ...
    return {
        ...
        total,
        gift: (cart.gift && typeof cart.gift === 'object')
            ? cart.gift
            : { enabled: false, type: 'physical', text: '' },
        free_shipping_bar: {
            minimum_amount: Number((freeShippingBar as EntityRecord).minimum_amount || 500),
            has_free_shipping: Boolean((freeShippingBar as EntityRecord).has_free_shipping),
...
const normalizeOrderForTemplate = (order: EntityRecord, currency: string): EntityRecord => {
    const orderId = asString(order.id) || 'order-vtdr';
    const referenceId = asString(order.reference_id) || `VTDR-${orderId.slice(-6)}`;
    ...
    return {
        ...
        url: normalizeEntityUrl(order.url, `/customer/orders/${orderId}`, ['/order', '/orders']),
...
const splitCustomerName = (value: unknown): { firstName: string; lastName: string } => {
...
const normalizeUserForCustomerPages = (store: EntityRecord, order: EntityRecord | null): EntityRecord => {
    const fallbackCustomer = order && typeof order.customer === 'object'
        ? (order.customer as EntityRecord)
        : {};
...
const normalizeOrderForCustomerTemplate = (order: EntityRecord, currency: string): EntityRecord => {
    const base = normalizeOrderForTemplate(order, currency);
    const statusName = asString((order.status as EntityRecord | undefined)?.name || order.status) || 'جديد';
    const statusColor = asString((order.status as EntityRecord | undefined)?.color) || '#0ea5e9';
    const statusIcon = asString((order.status as EntityRecord | undefined)?.icon) || 'sicon-packed-box';
    const shippingRaw = (order.shipping && typeof order.shipping === 'object')
    ...
    const orderItems = asArray(order.items).map((item, index) => {
        ...
        return {
            ...
            product: {
                ...
                url: asString(product.url || item.url) || `/products/${asString(item.product_id || item.id || index + 1)}`,
                type: asString(product.type) || 'product'
            },
            options: Array.isArray(item.options) ? item.options : [],
            attachments: Array.isArray(item.attachments) ? item.attachments : [],
            product_reservations: Array.isArray(item.product_reservations) ? item.product_reservations : [],
            detailed_offers: Array.isArray(item.detailed_offers) ? item.detailed_offers : []
        } as EntityRecord;
    });
    ...
    return {
        ...
        shipping: {
            ...
            name: asString(shippingRaw.name || shippingRaw.method_name) || 'شحن قياسي',
        ...
        packages: [
            {
                shipping_company: {
                    name: asString(shippingRaw.name || shippingRaw.method_name) || 'شركة الشحن',
                    logo: asString(shippingRaw.logo) || undefined,
                    number: asString(shippingRaw.number),
                    tracing_link: asString(shippingRaw.tracing_link || shippingRaw.tracking_url)
                },
                branch: null,
                is_delivered: ['delivered', 'completed'].includes(asString(order.status).toLowerCase()),
                status: {
                    name: statusName,
                    icon: statusIcon,
                    color: statusColor
        ...
        links: Array.isArray(order.links) && order.links.length > 0
            ? order.links
            : [
                {
        ...
        is_pending_payment: asString(order.payment_status).toLowerCase() === 'pending',
        pending_payment_ends_in: Number(order.pending_payment_ends_in || 0),
        is_rated: Boolean(order.is_rated),
        rating: (order.rating && typeof order.rating === 'object') ? order.rating : null
    };
};
...
const normalizeCheckoutForTemplate = (
    ...
    const rawShippingMethods = asArray(checkout.available_shipping_methods).map((entry) => ({
        id: asString(entry.id),
    ...
    const shippingMethods = rawShippingMethods.length > 0
        ? rawShippingMethods
        : [
            { id: 'standard', name: 'شحن قياسي', description: 'التوصيل خلال 2-4 أيام عمل', cost: 0, currency },
            { id: 'express', name: 'شحن سريع', description: 'التوصيل خلال 24 ساعة', cost: 25, currency },
            { id: 'pickup', name: 'استلام من الفرع', description: 'استلام ذاتي من نقطة البيع', cost: 0, currency }
        ];

    const rawPaymentMethods = asArray(checkout.available_payment_methods).map((entry) => ({
        id: asString(entry.id),
    ...
    const customerRaw = (checkout.customer && typeof checkout.customer === 'object') ? (checkout.customer as EntityRecord) : {};
    const customer = {
        name: asString(customerRaw.name),
        email: asString(customerRaw.email),
        mobile: asString(customerRaw.mobile)
    };
    const addressRaw = (checkout.address && typeof checkout.address === 'object') ? (checkout.address as EntityRecord) : {};
    const address = {
    ...
    const shippingRaw = (checkout.shipping && typeof checkout.shipping === 'object') ? (checkout.shipping as EntityRecord) : {};
    const selectedShipping = shippingMethods.find((entry) => entry.id === asString(shippingRaw.method_id)) || null;
    const shippingCost = readMoneyAmount(shippingRaw.cost ?? (selectedShipping ? selectedShipping.cost : 0));
    const shipping = {
        method_id: selectedShipping ? selectedShipping.id : asString(shippingRaw.method_id),
        method_name: selectedShipping ? selectedShipping.name : asString(shippingRaw.method_name),
        cost: shippingCost,
        currency
    };

    const paymentRaw = (checkout.payment && typeof checkout.payment === 'object') ? (checkout.payment as EntityRecord) : {};
    const selectedPayment = paymentMethods.find((entry) => entry.id === asString(paymentRaw.method_id)) || null;
    const payment = {
        method_id: selectedPayment ? selectedPayment.id : asString(paymentRaw.method_id),
        method_name: selectedPayment ? selectedPayment.name : asString(paymentRaw.method_name),
        type: selectedPayment ? selectedPayment.type : asString(paymentRaw.type)
    };

    const summaryRaw = (checkout.summary && typeof checkout.summary === 'object') ? (checkout.summary as EntityRecord) : {};
    const summary = {
        subtotal: readMoneyAmount(summaryRaw.subtotal || cart.sub_total),
        options_total: readMoneyAmount(summaryRaw.options_total || cart.options_total),
        ...
        total: readMoneyAmount(summaryRaw.total || (readMoneyAmount(cart.sub_total) + readMoneyAmount(cart.options_total) - readMoneyAmount(cart.total_discount) + readMoneyAmount(cart.tax_amount) + shippingCost)),
        currency
    };

    const addressDone = Boolean(customer.name && customer.email && customer.mobile && address.city && address.street && address.country);
    ...
    const requestedStep = validSteps.includes(forcedStep) ? forcedStep : asString(checkout.step || 'address');
    const maxReachableStep = !addressDone
        ? 'address'
        : (!shippingDone ? 'shipping' : (!paymentDone ? 'payment' : 'review'));
    const stepOrder: Record<string, number> = { address: 0, shipping: 1, payment: 2, review: 3 };
    ...
    const steps = validSteps.map((id, index) => ({
        ...
        is_done:
            ...
            (id === 'review' && (addressDone && shippingDone && paymentDone && Number(cart.count || 0) > 0))
    }));

    const canConfirm = addressDone && shippingDone && paymentDone && Number(cart.count || 0) > 0;
...
export const applyPreviewContext = (context: Record<string, unknown>, target: ResolvedPreviewTarget, query: Record<string, unknown>) => {
    ...
    const products = asArray(context.products).map((entry) => normalizeProductForTemplate(entry, defaultCurrency));
    const categories = asArray(context.categories).map((entry) => normalizeCategoryForTemplate(entry));
    const brands = asArray(context.brands).map((entry) => normalizeBrandForTemplate(entry));
    const offersById = new Map<string, EntityRecord>();
    for (const source of [asArray(context.offers), asArray(context.specialOffers), asArray((context as EntityRecord).special_offers)]) {
        for (const entry of source) {
            const normalized = normalizeOfferForTemplate(entry, defaultCurrency);
            const key = asString(normalized.id || normalized.slug) || `offer-${offersById.size + 1}`;
            offersById.set(key, normalized);
        }
    }
    const offers = Array.from(offersById.values());
    const blogArticles = asArray(context.blog_articles).map((entry) => normalizeArticleForTemplate(entry));
    const blogCategories = asArray(context.blog_categories).map((entry) => normalizeBlogCategoryForTemplate(entry));
    const pages = asArray(context.pages).map((entry) => normalizeStaticPageForTemplate(entry));
    ...
    const normalizedOrders = asArray(context.orders).map((entry) => normalizeOrderForCustomerTemplate(entry, defaultCurrency));
    const rawCart = (context.cart && typeof context.cart === 'object') ? (context.cart as EntityRecord) : {};
    const normalizedCart = normalizeCartForTemplate(rawCart, defaultCurrency);
    const rawCheckout = (context.checkout && typeof context.checkout === 'object') ? (context.checkout as EntityRecord) : {};
    const requestedStepGlobal = asString(query.step || query.checkout_step).toLowerCase();
    ...
    if (target.pageId === 'checkout') {
        ...
        const rawCheckout = (context.checkout && typeof context.checkout === 'object') ? (context.checkout as EntityRecord) : {};
        const normalizedCart = normalizeCartForTemplate(rawCart, defaultCurrency);
        const requestedStep = asString(query.step || query.checkout_step).toLowerCase();
        const normalizedCheckout = normalizeCheckoutForTemplate(rawCheckout, normalizedCart, defaultCurrency, requestedStep);
    ...
    if (target.pageId === 'thank-you') {
        ...
        const selectedOrder = resolveEntityByRef(orders, requestedOrderRef) || orders[orders.length - 1] || {
            id: 'order-vtdr',
        ...
        context.thank_you_title = asString((context as any).thank_you_title) || 'شكرًا لتسوقك معنا';
        context.page = {
            ...page,
    ...
    if (target.pageId.startsWith('customer/')) {
        ...
        const normalizedOrders = ordersRaw.map((entry) => normalizeOrderForCustomerTemplate(entry, defaultCurrency));
        const requestedOrderRef = asString(query.order_id || query.orderId || target.entityRef);
        const selectedOrder = resolveEntityByRef(normalizedOrders, requestedOrderRef) || normalizedOrders[0] || null;
        const storeEntityForUser = (context.store && typeof context.store === 'object')
            ? (context.store as EntityRecord)
            : {};
        const user = normalizeUserForCustomerPages(storeEntityForUser, selectedOrder);

        const notifications = asArray(context.notifications);
        const normalizedNotifications = (notifications.length > 0
            ? notifications
            : normalizedOrders.slice(0, 10).map((order, index) => ({
                ...
                url: asString(order.url) || `/customer/orders/${asString(order.id)}`,
                is_new: index === 0,
                date: asString(order.updated_at || order.created_at)
            }))) as EntityRecord[];
        (normalizedNotifications as EntityRecord[] & { next_page: string | null }).next_page = null;

        context.user = user;
        context.customer = normalizeCustomerAliasFromUser(user);
        context.orders = normalizedOrders as unknown as EntityRecord[];
        context.notifications = normalizedNotifications as unknown as EntityRecord[];
        context.custom_fields = Array.isArray(context.custom_fields) ? context.custom_fields : [];

        if (target.pageId === 'customer/orders/single') {
            const order = selectedOrder || normalizeOrderForCustomerTemplate({}, defaultCurrency);
            context.order = order;
            context.page = {
                ...page,
    ...
    if (target.pageId === 'product/single') {
        ...
        const similarProducts = resolveRelatedProductsForPreview(product, products, 12);
        ...
        context.page = {
            ...
            slug: 'product.single',
            url: asString(product.url) || `/products/${asString(product.id) || target.entityRef || ''}`
        };
        return;
    }

    if (target.pageId === 'product/index') {
        const requestedCategoryRef = asString(
            query.category_id ||
            query.categoryId ||
            query.category ||
            target.entityRef
        );
        const selectedCategory = requestedCategoryRef
            ? resolveEntityByRef(categories, requestedCategoryRef)
            : null;
        const searchQuery = asString(query.search) || asString(query.q);
        const selectedSort = asString(query.sort || query.sort_by || query.sortBy || 'ourSuggest') || 'ourSuggest';
        const sortOptions = [
        ...
        context.search_query = searchQuery || undefined;
        context.sort_options = sortOptions;

        context.page = {
            ...page,
            id: asString((selectedCategory || {}).id) || 'products',
            template_id: 'product/index',
            title: asString((selectedCategory || {}).name) || 'المنتجات',
            slug: 'product.index',
            url: asString((selectedCategory || {}).url) || (selectedCategory ? `/categories/${asString((selectedCategory || {}).id)}` : '/products')
        };
        return;
    }
    ...
    if (target.pageId === 'brands/single') {
        ...
        context.brand = {
            ...
            url: asString(brand.url) || `/brands/${asString(brand.id) || target.entityRef || ''}`,
    ...
    if (target.pageId === 'blog/index') {
        const selectedCategory = resolveEntityByRef(blogCategories, target.collectionRef);
        ...
        const categories = blogCategories.map((category) => {
            ...
            return {
                ...
                url: asString(category.url) || `/blog/categories/${asString(category.slug || categoryId)}`,
        ...
        const filtered = selectedCategory
            ? blogArticles.filter((article) => {
                const directCategory = asString(article.category_id || article.category);
                const categoryObjectId = asString(((article.category as EntityRecord | undefined) || {}).id);
                const targetId = asString((selectedCategory || {}).id);
                return directCategory === targetId || categoryObjectId === targetId;
            })
            : blogArticles;

        const articleList = [...filtered] as (EntityRecord[] & { next_page: string | null });
        ...
        context.page = {
            ...page,
            id: asString((selectedCategory || {}).id) || 'blog',
            template_id: 'blog/index',
            title: asString((selectedCategory || {}).name) || 'المدونة',
            slug: selectedCategory ? 'blog.category' : 'blog.index',
    ...
    if (target.pageId === 'blog/single') {
        const article = resolveEntityByRef(blogArticles, target.entityRef) || normalizeArticleForTemplate({});
        const articleId = asString(article.id);
        const related = blogArticles.filter((entry) => asString(entry.id) !== articleId).slice(0, 4);
        ...
        context.page = {
            ...
            title: asString(article.title) || 'مقالة',
            slug: 'blog.single',
            url: asString(article.url) || `/blog/${articleId || target.entityRef || ''}`
        };
        return;
    }
    ...
    if (target.pageId === 'page-single') {
        ...
        context.page = {
            ...
            url: asString(selected.url) || `/pages/${asString(selected.slug || selected.id || target.entityRef || 'page')}`,
            content: asString(selected.content) || '<p>هذه صفحة افتراضية من بيئة المعاينة.</p>'
        };
    }
};
...
export const bindPreviewContext = (input: {
    ...
    applyPreviewContext(context as unknown as Record<string, unknown>, target, query);
    (context.page as Record<string, unknown>).template_id =
        asString((context.page as Record<string, unknown>).template_id) || target.pageId;
