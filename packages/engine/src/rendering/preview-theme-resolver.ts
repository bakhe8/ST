...
const toVersion = (
  entry: ThemeVersionLike | undefined,
): ThemeVersionLike | null => {
...
const matchVersion = (
  ...
  return (
    versions.find(
      (version) => normalize(version.version).toLowerCase() === token,
    ) ||
...
const pickStoreVersion = (
  versions: ThemeVersionLike[],
  storeThemeVersionId: string,
): ThemeVersionLike | null => {
  if (storeThemeVersionId) {
    const byId = versions.find(
      (version) => normalize(version.id) === storeThemeVersionId,
    );
    if (byId) return byId;
  }
...
export class PreviewThemeResolver {
  ...
  public async resolve(input: {
    ...
    if (requestedThemeId) {
      ...
      const requestedVersions = Array.isArray(requestedTheme.versions)
        ? requestedTheme.versions
        : [];
      ...
      const selectedByRequest = matchVersion(
        requestedVersions,
        requestedVersion,
      );
    ...
    const storeVersions = Array.isArray(storeTheme.versions)
      ? storeTheme.versions
      : [];
    ...
    const selectedStoreVersion = pickStoreVersion(
      storeVersions,
      storeThemeVersionId,
    );
