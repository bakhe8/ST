...
import { hydratePreviewEntities, renderPreviewWithFallback } from './preview-runtime-service.js';
...
export class PreviewRenderOrchestrator {
    ...
    private async ensurePreviewBackfill(storeId: string): Promise<void> {
        ...
        try {
            ...
            const totalAdded = Object.values(added).reduce((sum, value) => sum + Number(value || 0), 0);
            if (totalAdded > 0) {
                console.log(
            ...
            console.warn(`[Preview] Backfill skipped for store=${storeId}: ${error?.message || 'unknown error'}`);
        }
    }
    ...
    public async render(input: {
        ...
        const target = resolvePreviewTarget(input.query.page, String(input.wildcardPath || ''));
    ...
    public getRenderBaseline(limit = 50): PreviewRenderBaseline {
        ...
        const avgTotalMs = roundMs(totalValues.reduce((acc, item) => acc + item, 0) / sample.length);
        const avgRenderMs = roundMs(renderValues.reduce((acc, item) => acc + item, 0) / sample.length);
    ...
    public async buildStoreContext(storeId: string): Promise<RuntimeContextResult> {
